...
internal class KeysWC
{
	...
	private string Search(string fn)
	{
		...
		{
			if (Operators.CompareString(tov[0, i], fn, TextCompare: false) == 0)
			{
	...
	private bool Reg()
	{
		...
		if (Operators.CompareString(type, "WebCheck", TextCompare: false) == 0)
		{
			if (Operators.CompareString(iniHGB.GetString("wc", "wc"), "WebCheck", TextCompare: false) != 0)
			{
				return false;
			}
			return true;
