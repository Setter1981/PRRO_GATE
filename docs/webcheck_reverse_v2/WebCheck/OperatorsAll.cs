...
internal class OperatorsAll
{
	...
	internal bool OperatorDefault(int idN)
	{
		...
		if (Microsoft.VisualBasic.CompilerServices.Operators.CompareString(operInfa2.INN[0].ToString().ToUpper(), "R", TextCompare: false) == 0)
		{
			if ((All.A.TypWork < 2019) | (All.A.TypWork > 2020))
			{
				Interaction.MsgBox("Резервний ключ не може бути оператором за замовчанням", MsgBoxStyle.Exclamation, "Увага");
			}
			return false;
		}
		UPDATE("1", operInfa2);
	...
	internal int IdOperatorToNm(int idOp)
	{
		...
		{
			if (Microsoft.VisualBasic.CompilerServices.Operators.CompareString(Op[0, i], idOp.ToString(), TextCompare: false) == 0)
			{
				return i;
			}
		}
