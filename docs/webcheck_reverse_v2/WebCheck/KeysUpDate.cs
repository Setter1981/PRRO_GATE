...
internal class KeysUpDate
{
	...
	internal bool DownloadKeyFile(string TINstr)
	{
		...
		try
		{
			MyProject.Computer.Network.DownloadFile(text, text2);
			if (File.Exists(text2))
			{
				string text3 = new KeysWC().DhgbK("temp", All.A.FN).ToString();
				if (text3.Length <= 2)
				{
					result = false;
					goto IL_0114;
				}
				if ((Operators.CompareString(Conversions.ToString(text3[0]), "2", TextCompare: false) != 0) | (Operators.CompareString(Conversions.ToString(text3[1]), "0", TextCompare: false) != 0))
				{
					result = false;
					goto IL_0114;
				}
				DeleteFil(strPathFile);
				FileSystem.RenameFile(text2, newName);
			}
		}
		catch (Exception ex)
		{
				...
				for (int i = 1; i <= num; i++)
				{
					...
					if (!DtinTrue(text2))
					{
						if (DownloadKeyFile(text2))
						{
							...
							dM = (string[])Utils.CopyArray(dM, new string[ZK + 1]);
							DM[ZK] = text2;
						}
	...
	private bool DtinTrue(string tinS)
	{
		...
		{
			if (Operators.CompareString(DM[i], tinS, TextCompare: false) == 0)
			{
				return true;
			}
		}
