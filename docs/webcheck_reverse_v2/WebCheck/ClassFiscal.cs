...
public class ClassFiscal
{
	...
	public bool Initialization(string strFN)
	{
		...
		if (parametrToString.errCode == 0 && Versioned.IsNumeric(parametrToString.ReturnStr) && Conversions.ToInteger(parametrToString.ReturnStr) == 1)
		{
	...
	public bool Finalization(string strFN)
	{
		...
		if (All.A.TypWork != 2025 && Operators.CompareString(All.f.StringGetFn(parametrToString.ReturnStr, "Path"), "", TextCompare: false) == 0)
		{
			parametrToString.errCode = 1001;
			parametrToString.errStr = "Фіскального номера " + parametrToString.ReturnStr + " у базі немає.";
			All.A.CurrentStatus = "Err=" + parametrToString.errCode;
		...
		if (Operators.CompareString(parametrToString.ReturnStr, All.A.FN, TextCompare: false) == 0)
		{
			All.A.SettingMode = false;
			All.A.Status = false;
			All.lC.StopControl();
	...
	public bool ReportZ(string strXML)
	{
		...
		if (All.A.Status)
		{
			...
			if (Operators.CompareString(All.d.GetParametrToString(strXML, "shiftCashInOut").ReturnStr, "1", TextCompare: false) == 0)
			{
			...
			All.A.ErrHelp = "";
			All.A.ErrCode = 0;
			All.Lg.SaveTextToLog("ReportZ " + now.ToString() + " - " + DateTime.Now.ToString(), strXML, StatusBarLog());
			return true;
		}
		parametrToString.errCode = 4;
		parametrToString.errStr = "Нет подключения";
	...
	public bool ReportX(string strXML)
	{
		...
		if (All.A.Status)
		{
			...
			All.A.ErrHelp = "";
			All.A.ErrCode = 0;
			All.Lg.SaveTextToLog("ReportX " + now.ToString() + " - " + DateTime.Now.ToString(), strXML, StatusBarLog());
			return true;
		}
		parametrToString.errCode = 4;
		parametrToString.errStr = "Нет подключения";
	...
	public bool GetCurrentStatus(string strFN)
	{
		...
		string text2 = ((Operators.CompareString(text, "", TextCompare: false) != 0) ? ("_LastOfflineStart=" + text + "_BalanceOffline=" + num) : checked((Operators.CompareString(All.A.FN, "7000000512", TextCompare: false) != 0) ? ("_LastOfflineStart=off_BalanceOffline=" + (10080 - All.f.IntegerGetFn(All.A.FN, "OfflineTime") - 9)) : ("_LastOfflineStart=DEMO_BalanceOffline=" + (10080 - All.f.IntegerGetFn(All.A.FN, "OfflineTime") - 9))));
		...
		else if (Operators.CompareString(All.A.FiscalMode, All.URLfact, TextCompare: false) == 0)
		{
			text4 = "_FiscalMode=1";
		}
	...
	public bool FiscalReceipt(string strXML)
	{
		...
		if (All.A.Status && Operators.CompareString(parametrToString.ReturnStr, All.A.FN, TextCompare: false) != 0)
		{
			parametrToString.errCode = 2;
		...
		All.A.ErrHelp = "";
		All.A.ErrCode = 0;
		All.Lg.SaveTextToLog("FiscalReceipt " + now.ToString() + " - " + DateTime.Now.ToString(), strXML, StatusBarLog());
		if (All.A.CheckTimeLog)
		{
	...
	public bool EPZtoCash(string strFN)
	{
		...
		if (!Versioned.IsNumeric(returnStr10))
		{
	...
	{
		All.l.OpenShiftReturn = true;
		if (Operators.CompareString(All.A.Report, "", TextCompare: false) != 0)
		{
		...
		if (Operators.CompareString(parametrToXML.ReturnStr.Trim(), "", TextCompare: false) == 0)
		{
	...
	public bool GetOfflineNumbers(string strXML)
	{
		...
		if (All.A.Status)
		{
			...
			int num = (Versioned.IsNumeric(parametrToString2.ReturnStr) ? Conversions.ToInteger(parametrToString2.ReturnStr) : 500);
			if (num > 2000)
			{
				num = 2000;
			}
	...
	public bool AddOperator(string strXML)
	{
		...
		if (All.A.Status)
		{
			...
			{
				if (!All.l.UpdateOperator(All.l.TextToTextXML(text), text2, text3, text4))
				{
					All.A.ErrHelp = "Помилка оновлення оператора";
					All.A.ErrCode = 83;
					All.A.CurrentStatus = "Err=" + All.A.ErrCode;
					All.Lg.SaveTextToLog("AddOperator", strXML, StatusBarLog());
					return false;
				}
				All.A.CurrentStatus = "Err=0_AddNewOperator=" + text + "_INN=" + text4 + "_UpDate=1";
				All.A.ErrHelp = "";
				All.A.ErrCode = 0;
				All.Lg.SaveTextToLog("AddOperator", strXML, StatusBarLog());
				return true;
			...
			{
				All.A.ErrHelp = "Помилка запису нового оператора на базу";
				All.A.ErrCode = 83;
				All.A.CurrentStatus = "Err=" + All.A.ErrCode;
				All.Lg.SaveTextToLog("AddOperator", strXML, StatusBarLog());
				return false;
			}
			All.A.CurrentStatus = "Err=0_AddNewOperator=" + text + "_INN=" + text4 + "_UpDate=0";
			All.A.ErrHelp = "";
			All.A.ErrCode = 0;
			All.Lg.SaveTextToLog("AddOperator", strXML, StatusBarLog());
			return true;
	...
	public bool AddPRRO(string strXML)
	{
		...
		if (!Versioned.IsNumeric(parametrToString.ReturnStr.Trim()))
		{
			parametrToString.errCode = 85;
	...
	public bool ShowWizardNewPro(string strFN = "")
	{
		...
		else if (!Versioned.IsNumeric(parametrToString.ReturnStr) | (parametrToString.ReturnStr.Length != 10))
		{
			parametrToString.ReturnStr = "";
		}
	...
	public bool ShowOfflineStatus()
	{
		if (All.A.Status)
		{
			...
			formOfflineStatus.ShowDialog();
			formOfflineStatus.Dispose();
			return true;
		}
		return false;
	}
	...
	public bool ShowDataRRO(string strFN = "")
	{
		if (!All.A.Status)
		{
			if (Operators.CompareString(strFN, "", TextCompare: false) == 0)
			{
	...
	public bool ShowReports(string strFN = "")
	{
		...
		formReports2.ShowDialog();
		formReports2.Dispose();
		return true;
	}
	...
	public bool ShowOperators(string strFN = "")
	{
		if (!All.A.Status)
		{
			...
			if (All.InitializationNew(strFN))
			{
				...
				formOperators.ShowDialog();
				formOperators.Dispose();
				All.FinalizationNew();
				return true;
			}
			return false;
		...
		formOperators2.ShowDialog();
		formOperators2.Dispose();
		return true;
	}
	...
	public bool ShowTestErrors()
	{
		if (All.A.Status)
		{
			...
			formTestErrors.ShowDialog();
			formTestErrors.Dispose();
			return true;
		}
		return false;
	}
	...
	public bool ShowWebchekFooterView()
	{
		...
		formWebchekFooterView.ShowDialog();
		formWebchekFooterView.Dispose();
		return true;
	}
	...
	public bool GetCheckByFiscalNumber(string strFN)
	{
		if (!All.A.Status)
		{
			if (All.InitializationNew(strFN))
			{
				...
				else if (Versioned.IsNumeric(parametrToString.ReturnStr))
				{
				...
				All.FinalizationNew();
				All.A.Report = Strings.Replace(typErrStr.ReturnStr, "mmmaaaccc", replacement);
				All.A.CurrentStatus = "FN=" + All.A.FN + "_Err=" + typErrStr.errCode;
				All.A.ErrHelp = typErrStr.errStr;
				All.A.ErrCode = typErrStr.errCode;
				return true;
		...
		int num = 0;
		num = ((parametrToString2.errCode > 0) ? 1 : ((!Versioned.IsNumeric(parametrToString2.ReturnStr)) ? 1 : Conversions.ToInteger(parametrToString2.ReturnStr)));
		...
		string replacement2 = "<MAC ID='" + typErrStr6.ReturnStr + "'>" + typErrStr5.ReturnStr + "</MAC>";
		All.A.Report = Strings.Replace(typErrStr4.ReturnStr, "mmmaaaccc", replacement2);
		All.A.CurrentStatus = "FN=" + All.A.FN + "_Err=" + typErrStr4.errCode;
		All.A.ErrHelp = typErrStr4.errStr;
		All.A.ErrCode = typErrStr4.errCode;
		return true;
	...
	public bool GetCheckcloudurl(string strXML)
	{
		...
		if (All.A.Status)
		{
			if (Operators.CompareString(typErrStr.ReturnStr, All.A.FN, TextCompare: false) != 0)
			{
				typErrStr.errCode = 2;
			...
			if ((Operators.CompareString(typPrintChecks.ReturnDocType, "0", TextCompare: false) == 0) | (Operators.CompareString(typPrintChecks.ReturnDocType, "1", TextCompare: false) == 0))
			{
	...
	{
		string s = "status=ok_version=6.0.8.1368";
	...
	public bool WriteSettingsINI(string sFN, string sName, string sValue)
	{
		...
		if (Operators.CompareString(sFN, All.A.FN, TextCompare: false) != 0)
		{
			return false;
		}
		if ((sFN.Length == 10) & Versioned.IsNumeric(sFN))
		{
	...
	public string GetSettingsINI(string sFN, string sName)
	{
		...
		return (!((sFN.Length == 10) & Versioned.IsNumeric(sFN))) ? All.f.GetString("Global", sName) : All.f.StringGetFn(sFN, sName);
	}
	...
	private bool AddAdvancePayment()
	{
		...
		num = ((!Versioned.IsNumeric(typErrStr.ReturnStr)) ? 999 : checked(Conversions.ToInteger(typErrStr.ReturnStr) + 1));
		string fN = All.A.FN;
		fN = ((Operators.CompareString(All.A.FiscalMode, "cabinet.tax.gov.ua:9443", TextCompare: false) != 0) ? All.A.FN : (fN + "_TS"));
	...
	{
		if (Operators.CompareString(All.f.GetString("Global", "OfflineReportZ"), "1", TextCompare: false) == 0)
		{
	...
	{
		if (Operators.CompareString(All.f.GetString("Global", "OfflineCheck"), "1", TextCompare: false) == 0)
		{
			string strXML = "<InputParameters><Parameters FN ='" + All.A.FN + "'/></InputParameters>";
			OnlineToOffline(strXML);
		}
	...
	public bool SendMessage(string strXML)
	{
		...
		int num = ((!Versioned.IsNumeric(parametrToString.ReturnStr)) ? 3 : Conversions.ToInteger(parametrToString.ReturnStr));
		if (num == 4)
		{
	...
	public bool InitializationFormal(string FNstr, string PathBase, string eTin)
	{
		...
		All.A.FN = FNstr.Trim();
		if (!Versioned.IsNumeric(All.A.FN) | (All.A.FN.Length != 10))
		{
			return false;
		}
		if (All.A.Status)
		...
		if (Operators.CompareString(All.A.FileN, "", TextCompare: false) == 0)
		{
			return false;
		}
		All.A.TIN = eTin;
