...
internal class CreateDB
{
	...
	public TypErr SavePayForms(string NameS, string IsCashS)
	{
		...
		if (Versioned.IsNumeric(typErrStr.ReturnStr) && checked(Conversions.ToInteger(typErrStr.ReturnStr) + 1) > 180)
		{
			result.errCode = 51;
	...
	internal TypErr UpdatePayForms(string eNamePay, string eNameOldPay, string eIsCashPay, string eidPay)
	{
		...
		result.errCode = 0;
		result.errStr = "";
		if (Operators.CompareString(eNameOldPay, eNamePay, TextCompare: false) != 0 && SearchPayForms(eNamePay))
		{
			result.errCode = 51;
			result.errStr = "Такое название платежа уже есть";
			return result;
