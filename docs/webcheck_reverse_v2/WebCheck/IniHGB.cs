...
internal class IniHGB
{
	...
	public string StringGetFn(string Fn, string Name)
	{
		...
		if (Operators.CompareString(Name.Trim().ToLower(), "path", TextCompare: false) == 0)
		{
			if (Operators.CompareString(text.Trim().ToLower(), "default", TextCompare: false) == 0)
			{
	...
	{
		if ((IFN > 0) & (Operators.CompareString(FNI, Fn, TextCompare: false) == 0))
		{
			return IFN;
		}
		int num = 1;
		...
		while (!flag)
		{
			...
			if (Operators.CompareString(Fn.Trim(), @string, TextCompare: false) == 0)
			{
				IFN = num;
				FNI = Fn;
				return num;
	...
	private int IndexNewFn()
	{
		...
		{
			string @string = GetString("Fns", num.ToString());
			if (Operators.CompareString(@string.Trim(), "del", TextCompare: false) == 0)
			{
				return num;
			}
			if (@string.Trim().Length > 0 && !Versioned.IsNumeric(@string))
			{
				return num;
			}
			if (Operators.CompareString(@string, "", TextCompare: false) == 0)
	...
	public int IndexMaxFn()
	{
		...
		checked
		{
			while (!flag)
			{
				if (Operators.CompareString(GetString("Fns", num.ToString()), "", TextCompare: false) == 0)
				{
					return num - 1;
				}
				num++;
