...
internal class HexTextBpos
{
	...
	internal bool Parsing(string HexStr)
	{
		...
		checked
		{
			...
			while (num <= 99);
			if ((Operators.CompareString(X[1], "13", TextCompare: false) == 0) & (Operators.CompareString(X[31].Trim(), "", TextCompare: false) == 0))
			{
	...
	private string Del06(string e)
	{
		...
		try
		{
			if ((Operators.CompareString(Conversions.ToString(e[0]), "0", TextCompare: false) == 0) & (Operators.CompareString(Conversions.ToString(e[1]), "6", TextCompare: false) == 0))
			{
