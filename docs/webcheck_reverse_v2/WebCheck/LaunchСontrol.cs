...
internal class LaunchСontrol
{
	...
	internal int YGet(string fn)
	{
		...
		try
		{
			...
			{
				if (Operators.CompareString(MestoF[num2], "0", TextCompare: false) == 0)
				{
					MestoF[num2] = fn;
					result = num2;
					goto IL_004d;
	...
	{
		if (Operators.CompareString(MestoF[index], fn, TextCompare: false) == 0)
		{
			MestoF[index] = "";
		}
	}
	...
	internal void StartControlForm(string fnControl)
	{
		try
		{
			...
			{
				FormTimer formTimer = new FormTimer(fnControl);
				if (!Environment.UserInteractive)
				{
					Application.Run(formTimer);
				}
				else
				{
					formTimer.Show();
