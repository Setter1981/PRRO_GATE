...
internal class NumbersOfflineUseRobot
{
	private readonly string Connection;

	private readonly string Fn;

	public NumbersOfflineUseRobot(string ConnectionS, string fnS)
	{
	...
	public TypErrStr OfflineID()
	{
		...
		else
		{
			try
			{
				...
				sQLiteDataReader.Close();
				sQLiteConnection.Close();
				if (Operators.CompareString(typErrStr.ReturnStr.Trim(), "", TextCompare: false) == 0)
				{
			...
			catch (Exception ex)
			{
				...
				Exception ex2 = ex;
				typErrStr.errCode = 35;
				typErrStr.errStr = "Помилка при отриманні номера для офлайн документа.";
				typErrStr.ReturnStr = "";
				result = typErrStr;
				ProjectData.ClearProjectError();
				goto IL_0162;
	...
	public TypErr UPDATEfns()
	{
		...
		catch (Exception ex)
		{
			ProjectData.SetProjectError(ex);
			Exception ex2 = ex;
			result.errStr = "Ошибка при попытке изменить запись в таблице fns";
			result.errCode = 90;
			ProjectData.ClearProjectError();
		}
		return result;
	...
	public TypErr MarkFNS(string fns)
	{
		TypErr result = default(TypErr);
		result.errCode = 0;
		result.errStr = "";
		fns = Strings.Replace(fns.Trim(), "`", "_");
		try
		{
			SQLiteConnection sQLiteConnection = new SQLiteConnection();
			new SQLiteCommand();
			...
			sQLiteConnection.Open();
			SQLiteCommand sQLiteCommand = sQLiteConnection.CreateCommand();
			sQLiteCommand.CommandText = "UPDATE fns SET used=datetime(CURRENT_TIMESTAMP, 'localtime') WHERE fns.checkidfiscal='" + fns + "'";
			SQLiteDataReader sQLiteDataReader = sQLiteCommand.ExecuteReader();
			((Component)(object)sQLiteCommand).Dispose();
			sQLiteDataReader.Close();
			sQLiteConnection.Close();
		...
		catch (Exception ex)
		{
			ProjectData.SetProjectError(ex);
			Exception ex2 = ex;
			result.errStr = "Ошибка при попытке отметить запись в таблице fns.";
			result.errCode = 90;
			ProjectData.ClearProjectError();
		}
		return result;
