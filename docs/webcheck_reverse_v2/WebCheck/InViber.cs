...
internal class InViber
{
	internal TypErr InTextViber(string nTax, string nPhone, int tSend)
	{
		...
		try
		{
			...
			string left = All.f.StringGetFn(All.A.FN, "PDF");
			...
			string text = LongDate(typPrintChecks.ReturnStr);
			...
			string text2 = "http://lic.webchek.com.ua/api.pl?";
			string text3 = "fn=" + All.A.FN;
			string text4 = "cid=" + nTax;
			string text5 = "dest=" + nPhone;
			string text6 = "ciddt=" + text;
			string text7 = "sm=" + All.Bablo(sBablo);
			string text8 = "messagetype=" + tSend;
			string text9 = ((Operators.CompareString(left, "1", TextCompare: false) != 0) ? "pdf=0" : "pdf=1");
			string text10 = "";
			text10 += SS(nTax, tSend);
			text10 += SS(All.A.FN, tSend);
			text10 += SS(nPhone, tSend);
			string text11 = "hashcid=" + text10;
			text2 = text2 + text3 + "&";
			text2 = text2 + text4 + "&";
			text2 = text2 + text5 + "&";
			text2 = text2 + text6 + "&";
			...
			text2 = text2 + text8 + "&";
			text2 += text11;
			All.Lg.SaveTextToLog("SendMessage", "Отправка чека на Vier", text4);
			string text12 = new WebClient().DownloadString(text2);
			...
			{
				text12 = text12.Trim();
				int num = ((!Versioned.IsNumeric(text12)) ? (-999) : Conversions.ToInteger(text12));
				int num2 = num;
				if (num2 > -1)
				{
					result.errCode = 0;
					result.errStr = text12;
				}
				else
				{
					switch (num2)
					...
					default:
					{
						string text13 = "Цей чек був відправлений ";
						if (Operators.CompareString(Conversions.ToString(text12[0]), "v", TextCompare: false) == 0)
						{
							text13 = text13 + Conversions.ToString(text12[9]) + Conversions.ToString(text12[10]) + "." + Conversions.ToString(text12[6]) + Conversions.ToString(text12[7]) + "." + Conversions.ToString(text12[1]) + Conversions.ToString(text12[2]) + Conversions.ToString(text12[3]) + Conversions.ToString(text12[4]);
							text13 = text13 + " " + Conversions.ToString(text12[12]) + Conversions.ToString(text12[13]) + ":" + Conversions.ToString(text12[15]) + Conversions.ToString(text12[16]);
							text13 += " VIBER";
							result.errCode = 91;
							result.errStr = text13;
						}
						else if (Operators.CompareString(Conversions.ToString(text12[0]), "s", TextCompare: false) == 0)
						{
							text13 = text13 + Conversions.ToString(text12[9]) + Conversions.ToString(text12[10]) + "." + Conversions.ToString(text12[6]) + Conversions.ToString(text12[7]) + "." + Conversions.ToString(text12[1]) + Conversions.ToString(text12[2]) + Conversions.ToString(text12[3]) + Conversions.ToString(text12[4]);
							text13 = text13 + " " + Conversions.ToString(text12[12]) + Conversions.ToString(text12[13]) + ":" + Conversions.ToString(text12[15]) + Conversions.ToString(text12[16]);
							text13 += " SMS";
						...
						{
							result.errCode = 91;
							result.errStr = "Получен ответ от сервера: " + text12;
						}
						break;
					}
					}
	...
	internal TypErr InTextViber()
	{
		...
		try
		{
			...
			{
				text5 = text5.Trim();
				int num = ((!Versioned.IsNumeric(text5)) ? (-999) : Conversions.ToInteger(text5));
				if (num > -1)
				{
					result.errCode = 0;
	...
	private TypErrStr TestNumPhone(string nPh)
	{
		...
		if (Operators.CompareString(Conversions.ToString(nPh[0]), "3", TextCompare: false) != 0)
		{
			result.errCode = 91;
			result.errStr = "Не верно указан номер телефона";
			return result;
		}
		if (Operators.CompareString(Conversions.ToString(nPh[1]), "8", TextCompare: false) != 0)
		{
			result.errCode = 91;
			result.errStr = "Не верно указан номер телефона";
			return result;
	...
	{
		strN = strN.Trim();
		if (Operators.CompareString(strN, "", TextCompare: false) == 0)
		{
			return "108";
		}
		checked
		{
			...
			for (int i = 0; i <= num3; i++)
			{
				...
				num2 = ((!Versioned.IsNumeric(text)) ? (num2 + (9 + intT + i)) : (num2 + (Conversions.ToInteger(text) + intT + i)));
			}
