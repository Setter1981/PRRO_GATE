...
internal class SQLrobot
{
	...
	{
		string returnStr = MaxID("ksef").ReturnStr;
		if (Operators.CompareString(returnStr, "0", TextCompare: false) == 0)
		{
			return;
		}
		...
		{
			DateTime date = Convert.ToDateTime(typErrKsefLast.ReturnDate);
			if (Math.Abs(DateAndTime.DateDiff(DateInterval.Minute, DateTime.Now, date)) >= 60 && Operators.CompareString(returnStr, typErrKsefLast.ReturnID, TextCompare: false) == 0 && ((Operators.CompareString(typErrKsefLast.ReturnTyp, "9", TextCompare: false) == 0) & (Operators.CompareString(typErrKsefLast.ReturnOffline, "2", TextCompare: false) == 0)))
			{
