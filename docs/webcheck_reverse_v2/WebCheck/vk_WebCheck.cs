...
public class vk_WebCheck : IInitDone, ILanguageExtender
{
	...
	{
		if (Operators.CompareString(bstrPropName, "vkFN", TextCompare: false) == 0 || Operators.CompareString(bstrPropName, "вкФН", TextCompare: false) == 0)
		{
			plPropNum = 0;
		}
		else
	...
	public void CallAsFunc(int lMethodNum, ref object pvarRetValue, ref Array paParams)
	{
		...
		try
		{
			...
			case 2:
			{
				...
				if (All.A.Status & (Operators.CompareString(pFNvk, All.A.FN, TextCompare: false) == 0))
				{
					text24 = All.A.FN;
					pvarRetValue = true;
				}
			...
			case 4:
			{
				string text21 = Conversions.ToString(paParams.GetValue(0));
				text21 = All.A.FN;
				paParams.SetValue(text21, 0);
			...
			case 5:
			{
				...
				text13 = Strings.Replace(text13, "\"", "'");
				string replacement = "fn='" + All.A.FN + "' operatorid";
				text13 = Strings.Replace(text13, "cashiervatin", replacement);
			...
			case 6:
			{
				...
				string text18 = Conversions.ToString(paParams.GetValue(1));
				string value7 = Conversions.ToString(paParams.GetValue(2));
				string text19 = Conversions.ToString(paParams.GetValue(3));
				string value8 = Conversions.ToString(paParams.GetValue(4));
				text19 = All.l.ReturnOpenShift().ReturnStr;
				int num6 = All.Rf.NumberOfChecks(Conversions.ToInteger(text19));
				text18 = text18.ToLower();
				text18 = Strings.Replace(text18, "\"", "'");
				string replacement2 = "fn='" + All.A.FN + "' operatorid";
				text18 = Strings.Replace(text18, "cashiervatin", replacement2);
				if (WebC.ReportZ(text18))
				{
					value6 = All.A.FN;
				...
				paParams.SetValue(text18, 1);
				paParams.SetValue(value7, 2);
				paParams.SetValue(text19, 3);
			...
			case 7:
			{
				...
				string text23 = Conversions.ToString(paParams.GetValue(1));
				double num7 = Conversions.ToDouble(paParams.GetValue(2));
				text23 = text23.ToLower();
				text23 = Strings.Replace(text23, "\"", "'");
				string replacement3 = "";
				if (num7 > 0.0)
				{
					replacement3 = "sumin='" + num7 + "' paymentid='1' fn='" + All.A.FN + "' operatorid";
				}
				else if (num7 < 0.0)
				{
					replacement3 = "sumout='" + Math.Abs(num7) + "' paymentid='1' fn='" + All.A.FN + "' operatorid";
				}
				text23 = Strings.Replace(text23, "cashiervatin", replacement3);
				if (WebC.CashInOut(text23))
				{
					pvarRetValue = true;
				}
				else
			...
			case 8:
			{
				...
				text6 = "ККТ";
				num = 2005;
				text7 = Conversions.ToString(Value: false);
				text8 = Conversions.ToString(Value: false);
			...
			case 13:
			{
				string text27 = Conversions.ToString(paParams.GetValue(0));
				...
				string text28 = Conversions.ToString(paParams.GetValue(4));
				text27 = All.A.FN;
				num9 = Conversions.ToInteger(All.l.ReturnOpenShift().ReturnStr);
				num8 = All.Rf.NumberOfChecks(num9);
				num10 = ((num9 <= 0) ? 1 : 2);
				text28 = "<?xml version='1.0' encoding='UTF-8'?><StatusParameters><Parameters BacklogDocumentsCounter='0' /></StatusParameters>";
				paParams.SetValue(text27, 0);
				paParams.SetValue(num8, 1);
				paParams.SetValue(num9, 2);
				paParams.SetValue(num10, 3);
				paParams.SetValue(text28, 4);
				pvarRetValue = true;
				break;
			}
			case 14:
			{
				...
				string text16 = Conversions.ToString(paParams.GetValue(5));
				string text17 = Conversions.ToString(paParams.GetValue(6));
				xmlVK = new Vk().XMLvkToCom(xmlVK);
				if (Operators.CompareString(All.d.GetParametrToString(xmlVK, "pc").ReturnStr.ToUpper(), "ВИДАЧА КОШТІВ", TextCompare: false) == 0)
				{
				...
				text16 = All.NumberTaxVk;
				text17 = "https://www.webchek.com.ua/";
				...
				paParams.SetValue(text16, 5);
				paParams.SetValue(text17, 6);
				break;
			}
			case 15:
			{
				string text20 = Conversions.ToString(paParams.GetValue(0));
				text20 = text20.Trim();
				text20 = text20.ToLower();
				string strFN = "<InputParameters><Parameters FN='" + pFNvk + "'/></InputParameters>";
				pvarRetValue = true;
				string left = text20;
				if (Operators.CompareString(left, "ShowWizardNewPro".ToLower(), TextCompare: false) == 0)
				{
					pvarRetValue = WebC.ShowWizardNewPro();
				}
				else if (Operators.CompareString(left, "ShowDataRRO".ToLower(), TextCompare: false) == 0)
				{
					pvarRetValue = WebC.ShowDataRRO(strFN);
				}
				else if (Operators.CompareString(left, "ShowOperators".ToLower(), TextCompare: false) == 0)
				{
					pvarRetValue = WebC.ShowOperators(strFN);
				}
				else if (Operators.CompareString(left, "ShowLicenseCheck".ToLower(), TextCompare: false) == 0)
				{
					pvarRetValue = WebC.ShowLicenseCheck();
				}
				else if (Operators.CompareString(left, "ShowReports".ToLower(), TextCompare: false) == 0)
				{
				...
				if (Operators.ConditionalCompareObjectEqual(pvarRetValue, false, TextCompare: false))
				{
					vkErrStr = All.A.ErrHelp;
					vkErrInt = All.A.ErrCode;
				}
				paParams.SetValue(text20, 0);
				break;
			}
			case 16:
			{
			...
			case 18:
			{
				...
				if (Operators.CompareString(text.Trim(), All.A.FN, TextCompare: false) == 0)
				{
