...
internal struct SubmitPtr
{
	internal TypErrSubmit SubmitCheck(string pathFile, string numbCheck, byte typCheck, long dd, string idCancel = "", string idOffline = "", bool OpenCloseShift = false)
	{
		...
		{
			int number;
			if (Versioned.IsNumeric(numbCheck))
			{
			...
			else
			{
				...
				if (Versioned.IsNumeric(lastLocalCheckNumbern))
				{
			...
			else if (Operators.CompareString(typErrSubmit.returnNumber.Trim(), "", TextCompare: false) == 0)
			{
				typErrSubmit.errCode = 25;
				typErrSubmit.errStr = "Помилка відправлення чека";
			}
	...
	internal TypErrSubmit LastCheck(string pathFile)
	{
		...
		checked
		{
			try
			{
				...
				for (int i = 1; i <= retries; i++)
				{
					...
					typErrSubmit.returnStr = answer.IdSign;
					typErrSubmit.returnData = answer.IdData;
					if (answer.Status == 1 || (answer.Status < 0 && answer.Status < -1 && answer.Status != -13))
					{
						break;
					}
				}
			...
			catch (Exception ex)
			{
				...
				Exception ex2 = ex;
				typErrSubmit.errCode = 25;
				typErrSubmit.errStr = "Ошибка отправки запроса на последний чек " + ex2.Message;
				result = typErrSubmit;
				ProjectData.ClearProjectError();
				goto IL_02e1;
			}
		}
		if ((typErrSubmit.returnStatus == 0) | (typErrSubmit.returnStatus == -1) | (typErrSubmit.returnStatus == -13))
		{
		...
		else if (Operators.CompareString(typErrSubmit.returnData.Trim(), "", TextCompare: false) == 0)
		{
			typErrSubmit.errCode = 0;
			typErrSubmit.errStr = "";
			All.Lg.SaveTextToLog("LastCheck", "Отправлен запрос для получения последнего чека", "Ответ = ПустаяСтрока");
		}
		else if (typErrSubmit.returnStatus == 1 && num > 1)
		{
			All.Lg.SaveTextToLog("LastCheck", "Виправлено помилку отримання останнього чека", "Спроб: " + num);
		}
		result = typErrSubmit;
		goto IL_02e1;
		IL_02e1:
	...
	private bool CheckZ(string PathFile)
	{
		...
		try
		{
			result = Operators.CompareString(PathFile[checked(PathFile.Length - 5)].ToString().ToLower(), "z", TextCompare: false) == 0;
		}
		catch (Exception ex)
		{
			ProjectData.SetProjectError(ex);
