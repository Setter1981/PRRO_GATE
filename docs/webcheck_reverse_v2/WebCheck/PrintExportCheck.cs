...
internal class PrintExportCheck
{
	...
	private System.Drawing.Image ImageQRt;
	...
	internal void PrintCheck(string NumCheckVis, System.Drawing.Image ImageQR, System.Drawing.Image ImageLogo)
	{
		ImageQRt = ImageQR;
		PrintLogo = ImageLogo;
	...
	internal void ExportToFile(string CheckXML, string CheckName)
	{
		...
		try
		{
			...
			if (All.A.ToPDF)
			{
				...
				{
					Microsoft.VisualBasic.FileIO.FileSystem.DeleteFile(path + CheckName + ".pdf");
			...
			if (All.A.ToXML)
			{
				...
				{
					Microsoft.VisualBasic.FileIO.FileSystem.DeleteFile(path + CheckName + ".xml");
					Application.DoEvents();
				}
			...
			if (All.A.ToTXT)
			{
				...
				{
					Microsoft.VisualBasic.FileIO.FileSystem.DeleteFile(path + CheckName + ".txt");
					Application.DoEvents();
				}
				ResCheck(CheckXML);
	...
	private void Fill(string CheckXML = "")
	{
		...
		checked
		{
			if (Operators.CompareString(CheckXML.Trim(), "", TextCompare: false) == 0)
			{
				typPrintChecks = ((Operators.CompareString(nPrint.ToLower(), "z", TextCompare: false) == 0) ? All.Rf.CheckXMLz() : (Versioned.IsNumeric(nPrint) ? All.Rf.CheckXMLNumber(nPrint, SearchID: true) : ((Operators.CompareString(nPrint.Trim(), "", TextCompare: false) == 0) ? All.Rf.CheckXMLNumber(nPrint) : (((Operators.CompareString(Conversions.ToString(nPrint[nPrint.Length - 1]), "Z", TextCompare: false) == 0) & (nPrint.Length < 9)) ? All.Rf.CheckXMLNumber(nPrint.ToUpper()) : ((!((Operators.CompareString(Conversions.ToString(nPrint[nPrint.Length - 1]), "z", TextCompare: false) == 0) & (nPrint.Length < 9))) ? All.Rf.CheckXMLNumberTax(nPrint) : All.Rf.CheckXMLNumber(nPrint.ToUpper()))))));
			}
			else
			{
			...
			Application.DoEvents();
			int num = 0;
			if (Operators.CompareString(nPrint, "LastX", TextCompare: false) == 0)
			{
				num = 4;
				Tb1 = "X ЗВІТ";
			...
			else if (Operators.CompareString(typPrintChecks.ReturnOffline, "0", TextCompare: false) == 0)
			{
				onOf = "онлайн";
			}
			switch (num)
	...
	private int TypChekcs(string xmlCheck)
	{
		...
		if (parametrToString.errCode == 0)
		{
			...
			if (Operators.CompareString(parametrToString.ReturnStr, "3", TextCompare: false) == 0)
			{
				return 3;
			}
	...
	private bool ExportToPDFnew(string PathPDF, string TaxNcheck)
	{
		...
		checked
		{
			try
			{
				...
				iTextSharp.text.Font font = new iTextSharp.text.Font(bf, 10f, 0);
				QrCodeImgControl qrCodeImgControl = new QrCodeImgControl();
				qrCodeImgControl.Width = 210;
				qrCodeImgControl.Height = 210;
				qrCodeImgControl.Text = CheckVis(TaxNcheck);
				ImageQRt = qrCodeImgControl.Image;
				iTextSharp.text.Image instance = iTextSharp.text.Image.GetInstance(ImageQRt, ImageFormat.Bmp);
				int num = LoadImg();
				iTextSharp.text.Image image = num switch
				{
					1 => iTextSharp.text.Image.GetInstance(PrintLogo, ImageFormat.Bmp), 
					2 => iTextSharp.text.Image.GetInstance(PrintLogo, ImageFormat.Jpeg), 
					3 => iTextSharp.text.Image.GetInstance(PrintLogo, ImageFormat.Png), 
					_ => null, 
				};
