...
internal class IniHGB
{
	...
	public void RenameFN(string OldName, string NewName)
	{
		...
		if (num <= 0)
		{
			...
			if (num > 0)
			{
				WriteString("Fns", Conversions.ToString(num), NewName);
			}
		}
	}
	...
	public void RenameTin(string OldName, string NewName)
	{
		...
		if (num <= 0)
		{
			...
			if (num > 0)
			{
				WriteString("Tins", Conversions.ToString(num), NewName);
			}
		}
	}
	...
	public void AddFn(string Fn)
	{
		...
		{
			num = IndexNewFn();
			WriteString("Fns", Conversions.ToString(num), Fn);
		}
	}
	...
	public void AddTin(string Tin)
	{
		...
		{
			num = IndexNewTin();
			WriteString("Tins", Conversions.ToString(num), Tin);
		}
	}
	...
	public void DelFn(string Fn)
	{
		...
		if (num > 0)
		{
			WriteString("Fns", Conversions.ToString(num), "del");
		}
	}
	...
	public void DelTin(string Tin)
	{
		...
		if (num > 0)
		{
			WriteString("Tins", Conversions.ToString(num), "del");
		}
	}
	...
	public int IndexFn(string Fn)
	{
		...
		while (!flag)
		{
			string @string = GetString("Fns", Conversions.ToString(num), "");
	...
	public int IndexTin(string Tin)
	{
		...
		while (!flag)
		{
			string @string = GetString("Tins", Conversions.ToString(num), "");
			if (Operators.CompareString(@string, "", false) == 0)
			{
				return 0;
			}
	...
	public int IndexMaxFn()
	{
		...
		checked
		{
			while (!flag)
			{
				if (Operators.CompareString(GetString("Fns", Conversions.ToString(num), ""), "", false) == 0)
				{
					return num - 1;
				}
				num++;
	...
	public int IndexMaxTin()
	{
		...
		checked
		{
			while (!flag)
			{
				if (Operators.CompareString(GetString("Tins", Conversions.ToString(num), ""), "", false) == 0)
				{
					return num - 1;
				}
				num++;
