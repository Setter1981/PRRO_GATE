...
internal class KeysWC
{
	...
	private readonly WebCheck.Wcwc Kod;
	...
	public int NumberRecords => checked(Z + 1);

	public KeysL LicenseKey
	{
		get
		{
			KeysL result = default(KeysL);
			result.Tin = "";
			result.Fn = "";
			result.Data = "";
			result.Tarif = "";
			...
			result.Tin = Tin;
			result.Fn = tov[0, index];
			result.Data = tov[1, index];
			result.Tarif = tov[2, index];
			return result;
		}
	}
	...
	private string KEYs(string value)
	{
		string result;
		try
		{
	...
	private long DhgbK(string tinS, string fnS)
	{
		Tin = tinS.Trim();
		if (!Dekode())
		{
	...
	public bool LoadKeys(string tinS)
	{
		Tin = tinS.Trim();
		return Dekode();
	}

	public long SearchDate(string fn)
	{
		long result = 0L;
		int z = Z;
		for (int i = 0; i <= z; i = checked(i + 1))
	...
	public KeysL SearchKey(string fn)
	{
		KeysL result = default(KeysL);
		result.Tin = "";
		result.Fn = "";
	...
	public long DateStrToInt64(string Dstr)
	{
		if (Dstr.Length == 10)
		{
	...
	private bool Reg()
	{
		WebCheck.IniHGB iniHGB = new WebCheck.IniHGB(WebCheck.All.MyDoc() + "\\test.ini");
	...
	{
		string path = WebCheck.All.MyDoc() + "\\WebCheck\\" + Tin + ".lic";
		bool result;
		try
		{
			File.WriteAllBytes(path, MF);
		}
		catch (Exception ex)
		{
			ProjectData.SetProjectError(ex);
