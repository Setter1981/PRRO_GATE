...
internal class ControlTime
{
	internal bool TimeContol(int MinutesT = 0)
	{
		...
		DateTime dateTime = StrToDt(text);
		if (Math.Abs(DateAndTime.DateDiff((DateInterval)8, now, dateTime, (FirstDayOfWeek)1, (FirstWeekOfYear)1)) > MinutesT)
		{
			return false;
		}
		return true;
