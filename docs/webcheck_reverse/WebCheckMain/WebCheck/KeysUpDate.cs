...
internal class KeysUpDate
{
	...
	internal bool DownloadKeyFile(string TINstr)
	{
		...
		try
		{
			...
			{
				string text4 = new KeysWC().DhgbK("temp", All.A.FN).ToString();
				if (text4.Length <= 2)
				{
					result = false;
					goto IL_0114;
				}
				if ((Operators.CompareString(Conversions.ToString(text4[0]), "2", false) != 0) | (Operators.CompareString(Conversions.ToString(text4[1]), "0", false) != 0))
				{
					result = false;
					goto IL_0114;
				}
