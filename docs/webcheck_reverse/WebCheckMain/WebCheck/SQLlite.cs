...
internal class SQLlite
{
	...
	{
		return Strings.Replace(Strings.Replace(txt, "'", "\\`", 1, -1, (CompareMethod)0), "\"", "\\\"", 1, -1, (CompareMethod)0);
	}
	...
	public TypErrStr ReturnOpenShift()
	{
		...
		if (!OpenShiftReturn & (Operators.CompareString(result.ReturnStr, "0", false) == 0))
		{
	...
	public TypErrStr ReturnOpenShiftEX(string FNs)
	{
		...
		if (Operators.CompareString(result.ReturnStr, "0", false) != 0)
		{
			...
			if (Operators.CompareString(result.ReturnStr, returnStr, false) != 0)
			{
				result.ReturnStr = "-1";
				result.errCode = 1003;
				result.errStr = "ВНИМАНИЕ! Открыто несколько смен!!!";
	...
	public TypErr CloseCurrentShift()
	{
		...
		if (Operators.CompareString(typErrStr.ReturnStr, "0", false) == 0)
		{
	...
	private void ErrorSaveSQL(string sSQL)
	{
		...
		do
		{
			if (Operators.CompareString(iniHGB.GetString("ErrorSQL", num.ToString()), "", false) == 0)
			{
	...
	public TypErrStr LastMac()
	{
		...
		if (Operators.CompareString(typErrStr.ReturnStr, "", false) == 0)
		{
			typErrStr.errCode = 30;
			typErrStr.errStr = "Ошибка получения предыдущего МАК";
			typErrStr.ReturnStr = "";
	...
	public TypShiftCheckID UidToTaxId(string eUID)
	{
		...
		result.ShiftID = "";
		eUID = eUID.Trim();
		if (Operators.CompareString(eUID, "", false) == 0)
		{
			return result;
		}
		try
		...
		checked
		{
			do
			{
				...
				if (blockS.id > 0)
				{
					...
					{
						if (Math.Abs((int)DateAndTime.DateDiff((DateInterval)9, blockS.SessionStartDT, DateTime.Now, (FirstDayOfWeek)1, (FirstWeekOfYear)1)) < 63)
						{
					...
					if (Math.Abs((int)DateAndTime.DateDiff((DateInterval)9, blockS.SessionStartDT, DateTime.Now, (FirstDayOfWeek)1, (FirstWeekOfYear)1)) > 81)
					{
						UpdateBlockStatus();
						result.errCode = 0;
						result.errStr = "";
	...
	public TypBackupInfo InfoBackup()
	{
		...
		if (Operators.CompareString(result.First, "", false) == 0)
		{
			result.First = "нема записів";
		}
		if (Operators.CompareString(result.Last, "", false) == 0)
		{
			result.Last = "нема записів";
		}
		return result;
	...
	{
		if (Operators.CompareString(All.A.FiscalMode, "cabinet.tax.gov.ua:9443", false) == 0 || !File.Exists(All.MyDoc() + "\\WebCheck\\Backup\\" + All.A.FN + ".db"))
		{
			return;
		}
		...
		do
		{
			...
			if (Operators.CompareString(typTransferBackup.id, "", false) != 0)
			{
	...
	{
		request = Strings.Replace(request, "'", "''", 1, -1, (CompareMethod)0);
