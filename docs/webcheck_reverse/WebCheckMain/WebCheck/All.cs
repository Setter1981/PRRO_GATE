...
{
	internal const string VersiyDLL = "6.0.7.1331";
	...
	internal static bool SecondRunControl(bool SRCb = true)
	{
		...
		if (!SRCb)
		{
			if (Operators.CompareString(SRC.Trim(), "", false) == 0)
			{
		...
		if (Operators.CompareString(SRC, f.StringGetFn(A.FN, "SRC").Trim(), false) == 0)
		{
			return true;
		}
	...
	internal static TypErrTimeShift ShiftTime()
	{
		...
		DateTime dateTime = Convert.ToDateTime(returnStr);
		DateTime now = DateTime.Now;
		checked
		{
			int num = (int)DateAndTime.DateDiff((DateInterval)8, dateTime, now, (FirstDayOfWeek)1, (FirstWeekOfYear)1);
			result.returnTime = 1440 - num;
			result.returnShiftStart = dateTime.ToString();
			...
			{
				result.errCode = 46;
				result.errStr = "ПРРО заблоковано. Зміна була відкрита " + dateTime.ToString() + " після чого минуло понад 24 години. Зніміть звіт Z.";
				result.returnShiftTime = false;
			}
			return result;
		}
	...
	internal static bool OfflineTimeOn()
	{
		...
		if (Operators.CompareString(A.FN, "7000000512", false) == 0)
		{
			return true;
		}
		...
		DateTime now = DateTime.Now;
		if (Math.Abs(DateAndTime.DateDiff((DateInterval)8, dateTime, now, (FirstDayOfWeek)1, (FirstWeekOfYear)1)) > checked(TimeLimOffline() - 9))
		{
			return false;
		}
		return true;
	...
	{
		if (Operators.CompareString(f.GetString("Global", "DTC"), "1", false) != 0)
		{
			return false;
		}
		return true;
	...
	internal static bool ControlSystemTime()
	{
		...
		DateTime now = DateTime.Now;
		if (DateAndTime.DateDiff((DateInterval)9, dateTime, now, (FirstDayOfWeek)1, (FirstWeekOfYear)1) < 0)
		{
			return false;
		}
		return true;
	...
	internal static long DateOfflineClose()
	{
		...
		if (!OfflineTimeOn())
		{
			...
			DateTime dateTime2 = Convert.ToDateTime(l.OfflineDate().ReturnStr.Trim());
			dateTime = DateAndTime.DateAdd((DateInterval)8, (double)num, dateTime2);
		}
	...
	internal static int RemTimeCurrentOffline()
	{
		...
		checked
		{
			...
			DateTime dateTime = Convert.ToDateTime(text);
			DateTime now = DateTime.Now;
			return TimeLimOffline() - (int)Math.Abs(DateAndTime.DateDiff((DateInterval)8, dateTime, now, (FirstDayOfWeek)1, (FirstWeekOfYear)1)) - 9;
		}
	}

	internal static string ReplacingQuotes(string txt)
	{
		txt = Strings.Replace(txt, "'", "\\'", 1, -1, (CompareMethod)0);
		txt = Strings.Replace(txt, "\"", "\\\"", 1, -1, (CompareMethod)0);
		return txt;
	}
	...
	internal static TypErrStr ReplacingAMP(string XMLs)
	{
		...
		result.errStr = "";
		result.ReturnStr = "";
		XMLs = Strings.Replace(XMLs, "&#34;", "```#34;", 1, -1, (CompareMethod)0);
		XMLs = Strings.Replace(XMLs, "&#38;", "```#38;", 1, -1, (CompareMethod)0);
		XMLs = Strings.Replace(XMLs, "&#39;", "```#39;", 1, -1, (CompareMethod)0);
		XMLs = Strings.Replace(XMLs, "&#60;", "```#60;", 1, -1, (CompareMethod)0);
		XMLs = Strings.Replace(XMLs, "&#62;", "```#62;", 1, -1, (CompareMethod)0);
		XMLs = Strings.Replace(XMLs, "&#46;", "```#46;", 1, -1, (CompareMethod)0);
		XMLs = Strings.Replace(XMLs, "&#8260;", "```#8260;", 1, -1, (CompareMethod)0);
		XMLs = Strings.Replace(XMLs, "&amp;#34;", "```#34;", 1, -1, (CompareMethod)0);
		XMLs = Strings.Replace(XMLs, "&amp;#38;", "```#38;", 1, -1, (CompareMethod)0);
		XMLs = Strings.Replace(XMLs, "&amp;#39;", "```#39;", 1, -1, (CompareMethod)0);
		XMLs = Strings.Replace(XMLs, "&amp;#60;", "```#60;", 1, -1, (CompareMethod)0);
		...
		XMLs = Strings.Replace(XMLs, "&amp;&lt;", "```#60;", 1, -1, (CompareMethod)0);
		XMLs = Strings.Replace(XMLs, "&amp;&gt;", "```#62;", 1, -1, (CompareMethod)0);
		XMLs = Strings.Replace(XMLs, "&amp;&period;", "```#46;", 1, -1, (CompareMethod)0);
		XMLs = Strings.Replace(XMLs, "&amp;&frasl;", "```#8260;", 1, -1, (CompareMethod)0);
		XMLs = Strings.Replace(XMLs, "&quot;", "```#34;", 1, -1, (CompareMethod)0);
		XMLs = Strings.Replace(XMLs, "&amp;", "```#38;", 1, -1, (CompareMethod)0);
		XMLs = Strings.Replace(XMLs, "&apos;", "```#39;", 1, -1, (CompareMethod)0);
		XMLs = Strings.Replace(XMLs, "&lt;", "```#60;", 1, -1, (CompareMethod)0);
	...
	internal static double NalOld()
	{
		...
		{
			if (Operators.CompareString(PayTax.get_ReportsPay(0, i).Trim(), "Готівка", false) == 0)
			{
				y = i;
				break;
			}
	...
	internal static TypErr ConnectDB()
	{
		...
		if (Operators.CompareString(A.FileN, "", false) == 0)
		{
			result.errCode = 1001;
		...
		if (Operators.CompareString(f.StringGetFn(A.FN, "FiscalMode"), "", false) == 0)
		{
			A.FiscalMode = URLfact;
			f.StringWriteFN(A.FN, "FiscalMode", "1");
		}
		if (Operators.CompareString(A.FiscalMode, "cabinet.tax.gov.ua:9443", false) == 0)
		{
			if (File.Exists(Strings.Replace(A.FileN, A.FN, A.FN + "_TS", 1, -1, (CompareMethod)0)))
			{
				A.Connection = Strings.Replace(A.Connection, A.FN, A.FN + "_TS", 1, -1, (CompareMethod)0);
			}
			else
			{
				A.FiscalMode = URLfact;
		...
		if (Operators.CompareString(f.StringGetFn(A.FN, "Acsksettings"), "", false) == 0)
		{
		...
		if (Versioned.IsNumeric((object)text3))
		{
		...
		if (Versioned.IsNumeric((object)text5))
		{
		...
		if (Operators.CompareString(f.StringGetFn(A.FN, "Offline"), "", false) == 0)
		{
			OfflineAllowed = true;
			f.StringWriteFN(A.FN, "Offline", "1");
		}
		else if (Operators.CompareString(f.StringGetFn(A.FN, "Offline"), "0", false) == 0)
		{
		...
		if (Versioned.IsNumeric((object)text6))
		{
		...
		if (!Versioned.IsNumeric((object)f.StringGetFn(A.FN, "OfflineMax")))
		{
			f.StringWriteFN(A.FN, "OfflineMax", "500");
		}
		if (!Versioned.IsNumeric((object)f.StringGetFn(A.FN, "OfflineMin")))
		{
		...
		if (Versioned.IsNumeric((object)text8))
		{
		...
		if (!Versioned.IsNumeric((object)text9.Trim()))
		{
			A.Multiplayer = true;
		...
		if (!Versioned.IsNumeric((object)f.StringGetFn(A.FN, "AllowableCash")))
		{
		...
		if (Operators.CompareString(f.StringGetFn(A.FN, "Showacquiring"), "1", false) == 0)
		{
			A.Showacquiring = true;
		}
		else if (Operators.CompareString(f.StringGetFn(A.FN, "Showacquiring"), "", false) == 0)
		{
			A.Showacquiring = true;
	...
	internal static string Bablo(float sBablo, bool sDot = true)
	{
		...
		return Strings.Replace(text, ".", "", 1, -1, (CompareMethod)0);
	}
	...
	{
		return Strings.Replace(NameS, ".", "#`#", 1, -1, (CompareMethod)0);
	}
	...
	internal static bool InitializationNew(string strFN)
	{
		...
		f.ClearIndexIni();
		if (Operators.CompareString(f.StringGetFn(parametrToString.ReturnStr, "Path"), "", false) == 0)
		{
	...
	public static bool SaveToFileText(string FileNameNow, string TextSave)
	{
		try
		{
			...
			{
				FileSystem.DeleteFile(FileNameNow);
			}
			Application.DoEvents();
			if (File.Exists(FileNameNow + ".p7s"))
			{
				FileSystem.DeleteFile(FileNameNow + ".p7s");
			}
			Application.DoEvents();
		}
		catch (Exception ex)
	...
	public static TypErrStr SubstitutePreviousMAC(string StrXML, string NumberShift, bool operINN = false)
	{
		...
		typErrMacNumDat.returnNumber = "";
		typErrMacNumDat.returnData = "";
		if (Operators.CompareString(MacTempOld, "", false) == 0)
		{
			typErrMacNumDat = ReturnLastCheckMac(NumberShift, operINN);
		...
		if (Operators.CompareString(MacTempOld, "error", false) == 0)
		{
	...
	internal static TypErrMacNumDat ReturnLastCheckMac(string NumberShift, bool operINN = false)
	{
		...
		if (Operators.CompareString(typErrSubmit.returnData.Trim(), "", false) == 0)
		{
			result.returnStr = "";
			return result;
		}
	...
	internal static TypErr ForbiddenSymbols(string strXML)
	{
		...
		result.errStr = "";
		strXML = strXML.Trim();
		if (Operators.CompareString(strXML, "", false) == 0)
		{
			result.errCode = 60;
	...
	{
		if (Strings.InStr(strXml, strP, (CompareMethod)0) > 0)
		{
			return true;
		}
		return false;
	...
	internal static string DelTabooS(string S)
	{
		...
		checked
		{
			...
			for (int i = 0; i <= num; i++)
			{
				...
				{
					text2 = Strings.Replace(text2, Conversions.ToString(text[i]), " ", 1, -1, (CompareMethod)0);
				}
			}
			return text2;
		}
	...
	internal static TypProductName DecoderProductName(string NameS)
	{
		...
		checked
		{
			...
			for (int i = 0; i <= num2; i++)
			{
				...
				if (Operators.CompareString(text + text2 + text3, "~`~", false) == 0)
				{
					num++;
					i += 2;
					continue;
	...
	internal static void DelIniDat()
	{
		...
		for (int i = 1; i <= num; i = checked(i + 1))
		{
			...
			if (Operators.CompareString(text, "del", false) != 0 && Operators.CompareString(text.Trim(), "", false) != 0)
			{
	...
	internal static bool AccessAWS()
	{
		...
		try
		{
			string text = "http://s3.eu-west-2.amazonaws.com/che.ck.ua/s3.txt";
			string text2 = PersonalTemp() + "s3.txt";
			if (File.Exists(text2))
			{
				FileSystem.DeleteFile(text2);
			}
	...
	internal static bool Money(string m)
	{
		...
		if ((m.Length == 1) & (Operators.CompareString(Conversions.ToString(m[0]), "0", false) == 0))
		{
			return false;
		}
		if ((m.Length == 2) & (Operators.CompareString(Conversions.ToString(m[0]), "00", false) == 0))
		{
			return false;
		}
		if ((m.Length == 3) & (Operators.CompareString(Conversions.ToString(m[0]), "000", false) == 0))
		{
			return false;
		}
		if (Operators.CompareString(Conversions.ToString(m[0]), "-", false) == 0)
		{
			return false;
		}
		return true;
	...
	internal static bool KeyReplacement(string innO)
	{
		...
		if (Operators.CompareString(iniHGB.GetString("R" + innO, "Updated"), "", false) == 0)
		{
			return false;
		}
