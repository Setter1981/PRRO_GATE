...
internal class databaseHGB
{
	...
	public databaseHGB(string FileName)
	{
		...
		Counter = 0;
		Uniqueness = true;
		if (Operators.CompareString(FileName.Trim(), "", false) != 0)
		{
			FileN = FileName;
		}
	}
	...
	public int SumInt(int Column)
	{
		...
		checked
		{
			...
			{
				if (Versioned.IsNumeric((object)M[Column, i]))
				{
	...
	public float SumSin(int ColumnSum, int ColumnSear, string Search)
	{
		...
		for (int i = 1; i <= counter; i = checked(i + 1))
		{
			if (Operators.CompareString(M[ColumnSear, i], Search, false) == 0 && Versioned.IsNumeric((object)M[ColumnSum, i]))
			{
	...
	public int SumInt(int Column, int ColumnSear, string Search)
	{
		...
		checked
		{
			for (int i = 1; i <= counter; i++)
			{
				if (Operators.CompareString(M[ColumnSear, i], Search, false) == 0 && Versioned.IsNumeric((object)M[Column, i]))
				{
					num += Convert.ToInt32(M[Column, i]);
				}
			}
	...
	public void Load()
	{
		...
		checked
		{
			try
			{
				...
				do
				{
					Array array = streamReader.ReadLine().Split(new char[1] { Conversions.ToChar(Sep) });
					if (!flag)
					{
						Counter++;
						ref string[,] m = ref M;
						m = (string[,])Utils.CopyArray((Array)m, (Array)new string[Col + 1, Counter + 1]);
					}
					flag = false;
					int col = Col;
					for (int i = 0; i <= col; i++)
					{
						M[i, Counter] = Conversions.ToString(NewLateBinding.LateIndexGet((object)array, new object[1] { i }, (string[])null));
					}
				}
	...
	public int Search(string text, int n)
	{
		...
		{
			if (Operators.CompareString(M[0, i], text.Trim(), false) == 0)
			{
				return i;
			}
		}
	...
	public int Search(string text, int colu, int n)
	{
		...
		for (int i = n; i <= counter; i = checked(i + 1))
		{
			if (Operators.CompareString(M[colu, i], text.Trim(), false) == 0)
			{
				return i;
			}
		}
