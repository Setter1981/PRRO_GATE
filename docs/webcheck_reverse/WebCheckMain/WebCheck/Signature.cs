...
internal class Signature
{
	...
	private void ReIni()
	{
		string text = All.MyDoc() + "\\WebCheck\\ospus.ini";
		...
		try
		{
			...
			{
				((ServerComputer)MyProject.Computer).FileSystem.CopyFile(text2, text);
			}
			Application.DoEvents();
		}
		catch (Exception ex)
	...
	private TypErrIntPrt LoadKey(string fileKeyS, string PasSport)
	{
		...
		if ((Operators.CompareString(KeyPasT, text2, false) != 0) | (Operators.CompareString(KeyPathT, text, false) != 0))
		{
			KeyDs = default(IntPtr);
		}
	...
	internal TypErrStrCert Cert(string KeyF, string PasS)
	{
		...
		try
		{
			...
			result.ReturnStr = Strings.Replace(certInfoEx.publicKeyID, " ", "", 1, -1, (CompareMethod)0);
		}
		catch (Exception ex)
		{
			ProjectData.SetProjectError(ex);
	...
	private bool KeyShiftTime(string ds)
	{
		...
		try
		{
			...
			DateTime dateTime = Conversions.ToDate(ds);
			result = ((checked((int)DateAndTime.DateDiff((DateInterval)8, now, dateTime, (FirstDayOfWeek)1, (FirstWeekOfYear)1)) > All.A.LimitCertificate) ? true : false);
		}
		catch (Exception ex)
		{
			ProjectData.SetProjectError(ex);
	...
	internal bool KeyShiftTimeINN(string INN)
	{
		...
		else
		{
			...
			{
				DateTime now = DateTime.Now;
				DateTime dateTime = Conversions.ToDate(new IniHGB(All.MyDoc() + "\\WebCheck\\Temp\\" + All.A.FN + "\\dat.ini").GetString(INN, "EndKey").Trim());
				result = ((checked((int)DateAndTime.DateDiff((DateInterval)8, now, dateTime, (FirstDayOfWeek)1, (FirstWeekOfYear)1)) > All.A.LimitCertificate) ? true : false);
			}
			catch (Exception ex)
			{
