...
internal class SendingOfflineChecksRobot
{
	...
	internal TypErrStr SendDoc()
	{
		...
		string idCancel = "";
		if (Operators.CompareString(typErrKsef.ReturnTyp, "1", false) == 0)
		{
			try
			{
		...
		checked
		{
			if (Operators.CompareString(typErrKsef.ReturnTyp.Trim(), "9", false) == 0 && Operators.CompareString(typErrKsef.ReturnID.Trim(), typErrMacNumDatDi.returnDI.ToString().Trim(), false) == 0 && Operators.CompareString(typErrKsef.ReturnNumber.Trim(), typErrMacNumDatDi.returnNumber.Trim(), false) != 0)
			{
				int num = (Versioned.IsNumeric((object)typErrKsef.ReturnID.Trim()) ? (Conversions.ToInteger(typErrKsef.ReturnID.Trim()) + 1) : 0);
			...
			if (Operators.CompareString(typErrKsef.ReturnNumber.Replace("`", "_"), typErrMacNumDatDi.returnNumber.Replace("`", "_"), false) == 0)
			{
				...
				typErr2.errStr = "";
				int num3;
				if (Operators.CompareString(typErrKsef.ReturnTyp, "9", false) == 0)
				{
					num3 = 1;
					do
					{
				...
				if (Operators.CompareString(typErrKsef.ReturnTyp, "80", false) != 0)
				{
	...
	private int NumberLC(string xmlLast)
	{
		...
		try
		{
			...
			result = (Versioned.IsNumeric((object)returnStr) ? Conversions.ToInteger(returnStr) : 0);
		}
		catch (Exception ex)
		{
			ProjectData.SetProjectError(ex);
