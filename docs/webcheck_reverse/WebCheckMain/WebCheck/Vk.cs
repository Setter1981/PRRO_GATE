	...
	internal string XMLvkToCom(string xmlVK)
	{
		...
		checked
		{
			...
			for (int j = 0; j <= num3; j++)
			{
				...
				if (Versioned.IsNumeric((object)array2[5, j]))
				{
			...
			string text8 = ((Operators.CompareString(text6, "", false) != 0) ? ("<Check FN='" + All.A.FN + "' OperationType='" + value + "' uuid='" + text6 + "'>") : ("<Check FN='" + All.A.FN + "' OperationType='" + value + "'>"));
			text8 += "<Goods>";
			int num4 = num2;
			...
			else
			{
				...
				for (int i = 0; i <= payN2; i++)
				{
					if (All.StrToDouble(array[i]) > 0.0)
					{
						...
						text8 = ((!((Operators.CompareString(text9, "1", false) == 0) & (Operators.CompareString(text7, "", false) != 0))) ? (text8 + "<Payment ID ='" + text9 + "' Sum='" + array[i] + "'/>") : (text8 + "<Payment ID ='" + text9 + "' Sum='" + array[i] + "' SMB='" + text7 + "'/>"));
					}
				}
			}
			text8 += "</Payments>";
