...
internal class StringXML
{
	...
	internal TypErr OpenShiftXML(string OperatorINN, string OperatorName)
	{
		...
		if (Versioned.IsNumeric((object)All.A.INN) && Convert.ToDouble(All.A.INN) > 0.0)
		{
			text3 = All.A.INN;
		}
		...
		text2 = Strings.Replace(text2, '\''.ToString(), '"'.ToString(), 1, -1, (CompareMethod)0);
	...
	private TypErr UidUn(string UidS)
	{
		...
		UidS = UidS.Trim();
		if (Operators.CompareString(UidS, "", false) == 0)
		{
			result.errCode = 88;
	...
	private TypErr Receipt()
	{
		...
		checked
		{
			...
			else
			{
				...
				if (!Versioned.IsNumeric((object)opertyp))
				{
					opertyp = "999";
				}
				...
				if (Operators.CompareString(idcancel.Trim(), "", false) != 0)
				{
				...
				try
				{
					if (Operators.CompareString(x.SelectSingleNode("/check/@fn").Value.Trim().ToLower(), All.A.FN.Trim().ToLower(), false) != 0)
					{
					...
					while (true)
					{
						if (num3 <= payN)
						{
							try
							{
								...
								if (Operators.CompareString(All.PayTax.get_PayName(num3).Trim(), "", false) == 0)
								{
									typErr.errCode = 62;
						...
						if (Operators.CompareString(All.f.StringGetFn(All.A.FN, "useecheckmegovua"), "1", false) == 0 && tegD.RRN.Length > 0)
						{
							if (Operators.CompareString(tegD.BID, "", false) == 0)
							{
								tegD.BID = tegD.RRN;
							}
							if (Operators.CompareString(tegD.RID, "", false) == 0)
							{
						...
						if (All.l.OfflineTrue())
						{
							...
							try
							{
								string text8 = Strings.Replace(xTAX, ".", "", 1, -1, (CompareMethod)0);
								text8 = Strings.Replace(text8, "#`#", ".", 1, -1, (CompareMethod)0);
								xTAX = Strings.Replace(xTAX, "#`#", ".", 1, -1, (CompareMethod)0);
								text8 = Strings.Replace(text8, "~`~", " # ", 1, -1, (CompareMethod)0);
								int num24 = Conversions.ToInteger(All.l.MaxID("CHECKHEAD").ReturnStr) + 1;
								typErr = All.l.SaveXMLcheckOffline(num24.ToString(), TaxDot(xTAX), TaxDot(text8), "not", typErrStr2.ReturnStr, opertyp, All.Bablo(num6.ToString()));
								if (typErr.errCode > 0)
								{
									result = typErr;
									break;
						...
						xTAX = Strings.Replace(xTAX, "~`~", " # ", 1, -1, (CompareMethod)0);
						string text9 = All.MyDoc() + "\\WebCheck\\Temp\\" + All.A.FN + "\\" + CheckIDv + "G.xml";
						xTAX = TaxDot(xTAX);
						All.SaveToFileText(text9, xTAX);
						...
						TypErr typErr3 = All.SF.SignatureFile(typErrOperKeyPass.KeyFile.Trim(), typErrOperKeyPass.Pass.Trim(), text9);
						...
						string pathFile = text9;
						text9 += ".p7s";
						TypErrSubmit typErrSubmit = submitPtr.SubmitCheck(text9, CheckIDv, 1, dd, idcancel);
						if (typErrSubmit.errCode > 0)
						{
						...
						try
						{
							...
							while (true)
							{
								if (num3 <= num29)
								{
									...
									if (Operators.CompareString(parametrToString4.ReturnStr, "1", false) == 0)
									{
	...
	{
		m = All.StrToDouble(Strings.FormatNumber((object)m, 1, (TriState)(-2), (TriState)(-2), (TriState)(-2)));
		if (smbO == 0.0)
		{
			return m;
		}
		string text = All.Bablo(smbO);
		string text2 = text[checked(text.Length - 2)].ToString();
		if ((Operators.CompareString(text2, "0", false) == 0) | (Operators.CompareString(text2, "5", false) == 0))
		{
			return Okruglit5(m);
		}
		return m;
	...
	{
		aaaS = aaaS.Trim();
		if (Operators.CompareString(aaaS, "", false) == 0)
		{
			return "'/>";
		}
		string text = "'>";
		checked
		{
			try
			{
				Array array = aaaS.Split(new char[1] { ',' });
				int num = array.Length - 1;
				for (int i = 0; i <= num; i++)
				{
					string text2 = Conversions.ToString(NewLateBinding.LateIndexGet((object)array, new object[1] { i }, (string[])null));
				...
				else
				{
					...
					else
					{
						...
						else
						{
							...
							if (Versioned.IsNumeric((object)text7))
							{
							...
							t[11] = text12;
							string text13 = t[4];
							if ((Operators.CompareString(t[7], "", false) != 0) | (Operators.CompareString(t[8], "", false) != 0))
							{
	...
	public TypErrStr GetParametrToString(string sXML, string name, string knot = "InputParameters/Parameters", bool RegUpLow = false)
	{
		...
		try
		{
			...
			name = name.Trim();
			knot = knot.Trim();
			if (!Information.IsNothing((object)xmlDocument.SelectSingleNode("/" + knot + "/@" + name)))
			{
	...
	internal string TegXml(string xmlOld)
	{
		...
		checked
		{
			...
			while (num <= 2);
			while (Strings.InStr(text, "=", (CompareMethod)0) > 0)
			{
				int num2 = Strings.InStr(text, "=", (CompareMethod)0) - 1;
				string text2 = "=";
				try
				{
					while (Operators.CompareString(Conversions.ToString(text[num2]), " ", false) != 0)
					{
						if (Operators.CompareString(Conversions.ToString(text[num2]), "ё", false) == 0)
						{
							text2 = "";
							break;
						}
				...
				if (Operators.CompareString(text2, "", false) != 0)
				{
					...
					text = Strings.Replace(text, text2, text3, 1, -1, (CompareMethod)0);
				}
				text = Strings.Replace(text, "=", "ёёё", 1, 1, (CompareMethod)0);
			}
			text = Strings.Replace(text, "ёёё", "=", 1, -1, (CompareMethod)0);
	...
	private string KnotBeginXml(string xmlOld)
	{
		...
		checked
		{
			...
			while (Strings.InStr(text, "<", (CompareMethod)0) > 0)
			{
				...
				try
				{
					while (Operators.CompareString(Conversions.ToString(text[num2]), " ", false) != 0 && Operators.CompareString(Conversions.ToString(text[num2]), ">", false) != 0)
					{
						num2++;
			...
			return Strings.Replace(text, "ёёё", "<", 1, -1, (CompareMethod)0);
		}
	}
	...
	private string KnotEndXml(string xmlOld)
	{
		...
		checked
		{
			...
			while (Strings.InStr(text, "</", (CompareMethod)0) > 0)
			{
				int num2 = Strings.InStr(text, "</", (CompareMethod)0);
				string text2 = "</";
				try
				{
					while (Operators.CompareString(Conversions.ToString(text[num2]), ">", false) != 0 && Operators.CompareString(Conversions.ToString(text[num2]), " ", false) != 0)
	...
	public TypErrStr GetParametrToXML(string s, bool zamena = true)
	{
		...
		checked
		{
			try
			{
				...
				Array array = s.Split(new char[1] { '_' });
				int num = array.GetLength(0) - 1;
				for (int i = 0; i <= num; i++)
				{
					Array array2 = (Array)NewLateBinding.LateGet(NewLateBinding.LateIndexGet((object)array, new object[1] { i }, (string[])null), (Type)null, "Split", new object[1] { "=" }, (string[])null, (Type[])null, (bool[])null);
					text = Conversions.ToString(Operators.AddObject(Operators.AddObject(Operators.AddObject(Operators.AddObject((object)(text + " "), NewLateBinding.LateIndexGet((object)array2, new object[1] { 0 }, (string[])null)), (object)"='"), NewLateBinding.LateIndexGet((object)array2, new object[1] { 1 }, (string[])null)), (object)"'"));
				}
				text += "/></OutputParameters>";
				text = text.Replace("`", "_");
				if (zamena)
				{
					typErrStr.ReturnStr = Strings.Replace(text, "'", '"'.ToString(), 1, -1, (CompareMethod)0);
				}
				else
				{
	...
	public TypErrStr ReportXZ(string sh, string tp = "x", string NumShift = "")
	{
		CheckTaxNum = "";
		CheckIDv = "";
		if (Operators.CompareString(tp, "x", false) == 0)
		{
			NumberShift = sh;
		}
		else
		...
		checked
		{
			...
			else
			{
				...
				while (true)
				{
					if (num2 <= taxN)
					{
						if (Operators.CompareString(payTax.get_ReportsSum(0, num2), "", false) != 0)
						{
							...
							{
								if (!Versioned.IsNumeric((object)payTax.get_ReportsSum(4, num2)))
								{
									payTax.set_ReportsSum(4, num2, "0");
								}
								if (!Versioned.IsNumeric((object)payTax.get_ReportsSum(1, num2)))
								{
									payTax.set_ReportsSum(1, num2, "0");
								}
								if (!Versioned.IsNumeric((object)payTax.get_ReportsSum(5, num2)))
								{
								...
								double num6 = 0.0;
								double num7 = 0.0;
								if (Versioned.IsNumeric((object)typErrOborot.SumTXA))
								{
									num5 = All.StrToDouble(typErrOborot.SumTXA);
								}
								if (Versioned.IsNumeric((object)typErrOborot.SumTXGA))
								{
									num6 = All.StrToDouble(typErrOborot.SumTXGA);
									num5 += num6 - num6 * 5.0 / 105.0;
								}
								if (Versioned.IsNumeric((object)typErrOborot.SumTXDA))
								{
								...
								num6 = 0.0;
								num7 = 0.0;
								if (Versioned.IsNumeric((object)typErrOborot.SumTXAret))
								{
									num5 = All.StrToDouble(typErrOborot.SumTXAret);
								}
								if (Versioned.IsNumeric((object)typErrOborot.SumTXGAret))
								{
									num6 = All.StrToDouble(typErrOborot.SumTXGAret);
									num5 += num6 - num6 * 5.0 / 105.0;
								}
								if (Versioned.IsNumeric((object)typErrOborot.SumTXDAret))
								{
							...
							{
								double num8 = ((!Versioned.IsNumeric((object)payTax.get_ReportsSum(4, num2))) ? 0.0 : All.StrToDouble(payTax.get_ReportsSum(4, num2)));
								if (Versioned.IsNumeric((object)payTax.get_ReportsSum(3, num2)))
								{
								...
								double num10 = ((!Versioned.IsNumeric((object)payTax.get_ReportsSum(5, num2))) ? 0.0 : All.StrToDouble(payTax.get_ReportsSum(5, num2)));
								double num11 = ((!Versioned.IsNumeric((object)payTax.get_ReportsSum(2, num2))) ? 0.0 : All.StrToDouble(payTax.get_ReportsSum(2, num2)));
								switch (text2)
								{
								case "4":
								{
					...
					{
						if (Operators.CompareString(payTax2.get_ReportsPay(0, num2), "", false) == 0)
						{
							continue;
						}
					...
					zX12 = zX12 + "<TS>" + dd + "</TS>";
					ZX += "</DAT>";
					if (All.l.OfflineTrue() && Operators.CompareString(tp.ToLower().Trim(), "z", false) == 0)
					{
						...
						try
						{
							string text6 = Strings.Replace(ZX, ".", "", 1, -1, (CompareMethod)0);
							text6 = Strings.Replace(text6, "#`#", ".", 1, -1, (CompareMethod)0);
							ZX = Strings.Replace(ZX, "#`#", ".", 1, -1, (CompareMethod)0);
							TypErr typErr2 = All.l.SaveXMLcheckOffline(sh.ToString() + "Z", TaxDot(ZX), TaxDot(text6), "not", typErrStr2.ReturnStr, "80");
							if (typErr2.errCode > 0)
							{
					...
					ZX = TaxDot(ZX);
					string text7 = All.MyDoc() + "\\WebCheck\\Temp\\" + All.A.FN + "\\" + sh + tp + ".xml";
					All.SaveToFileText(text7, ZX);
					if (Operators.CompareString(tp.ToLower().Trim(), "z", false) == 0)
					{
						if (Versioned.IsNumeric((object)NumShift.Trim()))
						{
							...
							TypErr typErr3 = All.SF.SignatureFile(typErrOperKeyPass.KeyFile.Trim(), typErrOperKeyPass.Pass.Trim(), text7);
							if (typErr3.errCode > 0)
							{
						...
						string pathFile = text7;
						text7 += ".p7s";
						CheckIDv = sh;
						TypErrSubmit typErrSubmit = submitPtr.SubmitCheck(text7, CheckIDv, 2, dd, "", "", OpenCloseShift: true);
						if (typErrSubmit.errCode > 0)
						{
				...
				else
				{
					...
					if (All.l.OfflineTrue())
					{
						...
						else
						{
							...
							else
							{
								try
								{
									string text4 = Strings.Replace(text2, ".", "", 1, -1, (CompareMethod)0);
									...
									TypErr typErr = All.l.SaveXMLcheckOffline(num2.ToString(), text2, text4, "not", typErrStr3.ReturnStr, text, All.Bablo(typErrStr.ReturnStr));
									if (typErr.errCode > 0)
									{
										typErrStr.errCode = typErr.errCode;
										typErrStr.errStr = typErr.errStr;
					...
					else
					{
						...
						else
						{
							...
							string text5 = All.MyDoc() + "\\WebCheck\\Temp\\" + All.A.FN + "\\" + num + ".xml";
							All.SaveToFileText(text5, text2);
							TypErrOperKeyPass typErrOperKeyPass = All.l.OperatorKeyPass(NumberShift);
							if (typErrOperKeyPass.errCode > 0)
							{
								typErrStr.errCode = typErrOperKeyPass.errCode;
							...
							{
								TypErr typErr2 = All.SF.SignatureFile(typErrOperKeyPass.KeyFile.Trim(), typErrOperKeyPass.Pass.Trim(), text5);
								if (typErr2.errCode > 0)
								{
									typErrStr.errCode = typErr2.errCode;
									typErrStr.errStr = typErr2.errStr;
								...
								{
									string pathFile = text5;
									text5 += ".p7s";
									CheckIDv = num.ToString();
									SubmitPtr submitPtr = default(SubmitPtr);
									TypErrSubmit typErrSubmit = submitPtr.SubmitCheck(text5, CheckIDv, 3, dd);
									if (typErrSubmit.errCode > 0)
									{
										typErrStr.errCode = typErrSubmit.errCode;
										typErrStr.errStr = typErrSubmit.errStr + "  Status: " + typErrSubmit.returnStatus + "  Msg: " + typErrSubmit.returnStr + "   ";
	...
	{
		string text = "^";
		taxS = ((!dot) ? Strings.Replace(taxS, ".", text, 1, -1, (CompareMethod)0) : Strings.Replace(taxS, text, ".", 1, -1, (CompareMethod)0));
		return taxS;
	}
	...
	internal TypErrStr NameDotNoDot(string XmlS, bool sDot, bool MacOn = true)
	{
		...
		if (!sDot)
		{
			XmlS = Strings.Replace(XmlS, ".", "", 1, -1, (CompareMethod)0);
		}
		XmlS = Strings.Replace(XmlS, "#`#", ".", 1, -1, (CompareMethod)0);
		if (MacOn)
		{
		...
		result.ReturnStr = Strings.Replace(XmlS, '\''.ToString(), '"'.ToString(), 1, -1, (CompareMethod)0);
		return result;
	}
	...
	internal TypErrStrUpdate UPloadXML()
	{
		...
		try
		{
			...
			result.ReturnStr = "6.0.7.1331";
			result.ReturnInf = result.ReturnInf.Replace("@@", "\r\n");
			if (VerInt("6.0.7.1331") < VerInt(result.ReturnVer))
			{
	...
	private int VerInt(string verStr)
	{
		...
		checked
		{
			...
			try
			{
				...
				if (Versioned.IsNumeric((object)verStr))
				{
