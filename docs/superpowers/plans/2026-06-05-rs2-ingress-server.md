...
5. **Stale-`PROCESSING` reaper** (re-review 2026-06-07, completeness lens). RS-3's `fiscalize` will CAS the inbox row NEW→`PROCESSING` (`acquire_lease`) before driving the write-path. A crash AFTER the lease but BEFORE a terminal state leaves the row stuck in `PROCESSING` — every retry of that idempotency key then resolves to `202 IN_PROGRESS` forever (the NEW-row release at `handler.rs` is guarded `WHERE status='NEW'`, so it cannot touch it). The replay matrix's "stuck-`PROCESSING` is retry-after-restart-able" assumption (H3) only holds if RS-3 builds a **boot/periodic reaper** that re-drives (or fails-out) rows left in `PROCESSING` past a threshold. The reaper reads the row's identity from the persisted columns (A-H1: `driver_id` + `signed_by_cashier_id`), so it is self-contained. **Obligation:** RS-3 ships the stale-`PROCESSING` reaper as a named deliverable, with a crash-mid-lease integration test. Tracked at the seam contract (`seam.rs` concurrency note) but listed here so it is not discovered late.
