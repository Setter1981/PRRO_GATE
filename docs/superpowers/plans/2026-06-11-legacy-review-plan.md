...
2. Fable: читает досье → перезапускает репро FT/HIGH/MED → дописывает rulings →
   мерджит docs-PR.
2b. **Только M1 и M3** (поправка 2026-06-11): после rulings Fable делает личный
   шов-проход — кросс-модульные стыки, которые модульная охота Opus видеть не
   может by construction (находки класса AUD-1 живут МЕЖДУ модулями):
   replay↔ledger↔boot-probe, drain↔shift-машинерия, A4-gate↔DB-lease.
   Результат — те же rulings-секции в досье. Ограниченный токен-спенд: швы —
   сотни строк, не тысячи.
