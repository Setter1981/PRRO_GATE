...
- **NEW** `rust/prro/src/services/offline_sync/kvt2_confirm.rs` — typed surface + helper:
  ...
  - `pub enum Kvt2ConfirmOutcome { Acked { kvt1_raw_bytes: Vec<u8>, sent_replay_trace_attempt_no: Option<i64> }, Hold { reason: Kvt2ConfirmHoldReason, sent_replay_trace_attempt_no: Option<i64> }, StructuralDrift { reason: Kvt2ConfirmStructuralReason, sent_replay_trace_attempt_no: Option<i64> }, SentNotFoundDowngrade { trace_attempt_no: i64 } }` — every outcome variant carries the optional `sent_replay_trace_attempt_no` (MED-PR70-R6-01 fix); `Some(_)` only when source == `SentReplay`; threads the Envelope 1c-pre allocated row into the completing envelope so EVERY outcome path completes the recovery trace row (boot parity per `boot_phase.rs:516,707,803`).  Sent-fresh and Kvt1 re-entry contexts always carry `None` because they do not allocate a W12-owned trace row (stage_send's 4-pre trace row is already managed by stage_send itself for Sent-fresh; Kvt1 re-entry has no probe-allocated row).  **LOW-PR70-R10-03 scope decision**: SentFresh path's lastChk wire call intentionally does NOT get a dedicated transport_trace row.  Rationale: forensic value of SentFresh lastChk timings is immediate-consume — Acked → `OFFLINE_DRAIN_KVT2_ADVANCED` audit captures the advance; Hold → next-tick SentReplay arm re-allocates a fresh recovery row, preserving forensic chain across ticks; StructuralDrift → `KVT2_CONFIRM_STRUCTURAL_DRIFT` audit + `BootError::Internal`.  No recovery context ever consults a SentFresh-time lastChk trace row, so the asymmetry has no behavioral impact.  If operator wants symmetric per-DPS-call tracing later, that becomes a separate forensic-hardening task (out of W12 scope).
  ...
  - `pub async fn confirm_drain_doc(pool: &SqlitePool, dps: &dyn DpsChannel, doc: &fiscal_documents::DocumentRow, expected_server_fiscal_no: &str, fn_sign: &CheckSignBlob, source: Kvt2ConfirmSource) -> Result<Kvt2ConfirmOutcome, BootError>` — source-aware helper.  Signature carries TWO separate inputs (MED-PR70-R11-01 fix):
    - `doc: &fiscal_documents::DocumentRow` — provides `backend_profile_id` + `transport_profile_id` for `transport_trace::NewAttempt` (LOW-PR70-R10-01 fix scope) AND the `document_id` for state CAS / audit; this is the **cohort-walker snapshot** which may be pre-stage_send (`doc.state == OfflineLocalAck | ErrorRetryable`, `doc.server_fiscal_no == None`) when source is `SentFresh`, so `doc.server_fiscal_no` MUST NOT be used as the lookup id.
    - `expected_server_fiscal_no: &str` — the canonical fresh DPS-stamped fiscal id, sourced per entry point (see "Expected-id handoff" below).  This is what helper passes to `dps.by_server_fiscal_no(fn_sign, expected_server_fiscal_no)`.
  - Same body for all three contexts but routes evidence outcomes per the **source-context mapping matrix** below.  Helper calls **`dps.by_server_fiscal_no(fn_sign, expected_server_fiscal_no)`** (HIGH-PR70-R8-01 fix; canonical typed lookup per `channel.rs:53-69`) OUTSIDE any `with_immediate` per I1.  This surface — NOT raw `last_chk` — is essential: `by_server_fiscal_no` performs the canonical empty-id → `DpsError::NotFound` mapping that the safe-redrive contract depends on; raw `last_chk` returns `Ok(CheckAck { id: "" })` on absent-history which would otherwise mis-route to the id-mismatch fail-loud arm.  For `Kvt2ConfirmSource::SentReplay`, the helper additionally allocates a `transport_trace` recovery row pre-DPS-call and threads its `attempt_no` into the `SentNotFoundDowngrade` variant (per MED-PR70-R5-02 lifecycle).

  ### Expected-id handoff per source context (MED-PR70-R11-01 fix)

  Where caller obtains `expected_server_fiscal_no` for each entry point:

  - **`Kvt2ConfirmSource::SentFresh`** (caller = `process_via_stage_send`): `expected_server_fiscal_no = &outcome.server_fiscal_no` where `outcome` is the just-returned `StageSendOutcome::Sent { server_fiscal_no, attempt_no }`.  **Not** `doc.server_fiscal_no` — at this call site `doc` is the pre-stage_send cohort-walker snapshot (per `backlog_drain.rs:822` + `backlog_drain.rs:859-861` docstring) and its `server_fiscal_no` is `None` for fresh OfflineLocalAck source.  The fresh stamped id lives in the outcome variant.
  - **`Kvt2ConfirmSource::SentReplay`** (caller = `process_via_lastchk_replay`): `expected_server_fiscal_no = doc.server_fiscal_no.as_deref().ok_or_else(|| structural_drift_server_fiscal_no_missing())?` — persisted cohort entry where `server_fiscal_no` MUST be Some per stage_send 4-b invariant (committed Sent state implies stamped id).  If `None` surfaces here, that's `StructuralDrift(ServerFiscalNoMissing)` → `BootError::Internal` BEFORE any DPS call.
  - **`Kvt2ConfirmSource::Kvt1Reentry`** (caller = `process_via_w12_only`): same as SentReplay — `expected_server_fiscal_no = doc.server_fiscal_no.as_deref().ok_or_else(...)?`; Kvt1 state implies prior Sent→Kvt1 advance which preserved the stamped id.  `None` → `StructuralDrift(ServerFiscalNoMissing)`.

  Helper itself never reads `doc.server_fiscal_no` for lookup — the caller is responsible for sourcing the canonical expected id from the correct frame (post-stage_send outcome for SentFresh; persisted row for SentReplay/Kvt1Reentry).  Helper trusts the input parameter.

  ### Helper vs caller envelope ownership (LOW-PR70-R10-02 fix)
  ...
  - `Hold { reason, sent_replay_trace_attempt_no }` — for SentReplay, Envelope 1c-hold ALREADY committed (trace.complete no-state-change + audit); doc state unchanged; caller projects to `DocVerdict::HoldFnDrain { projection }`.
  - `StructuralDrift { reason, sent_replay_trace_attempt_no }` — for SentReplay, Envelope 1c-drift ALREADY committed (trace.complete structural + audit); caller propagates `BootError::Internal`.
  - `SentNotFoundDowngrade { trace_attempt_no }` — Envelope 1c-post ALREADY committed (trace.complete TransientRetry + Sent→ER + audit); doc state is `ErrorRetryable`; caller projects to `DocVerdict::HoldFnDrain { projection: ErRedriveQueued }`.

  Caller (`process_via_stage_send` / `process_via_w12_only` / `process_via_lastchk_replay`) does ONLY:
  - Map outcome variant → `DocVerdict` with correct `HoldFnDrainProjection`.
  - Record into `DrainSummary` via projection-specific recording method.
  - Invoke `stage_finalize::run` (Envelope 2) on `Acked` for `Kvt2 → Ack`.
  - Propagate `BootError::Internal` on `StructuralDrift`.

  Rationale: helper-heavy ownership keeps state mutations encapsulated and atomic per outcome class; caller stays projection-only and easy to test against a synthetic helper stub.  Envelope 2 stays caller-owned because it is the M3a `stage_finalize::run` pool-only contract which W12 must not weaken (per `stage_finalize.rs:198-204` W8 review F1 docstring).
...
- **Commit 4 — Sent-source W12 wiring (Envelope 1a + Envelope 2)**: replace `apply_w12_confirmation` stub body — on `StageSendOutcome::Sent { server_fiscal_no, attempt_no }`, call `kvt2_confirm::confirm_drain_doc(pool, dps, doc, &outcome.server_fiscal_no, fn_sign, Kvt2ConfirmSource::SentFresh)` (MED-PR70-R11-01 fix: `expected_server_fiscal_no` sourced from the StageSendOutcome variant, NOT from the pre-stage_send cohort snapshot `doc.server_fiscal_no`); on `Acked` route through Envelope 1a (Kvt1Raw via `document_files::replace_tx` + `Sent → Kvt1` CAS + `Kvt1 → Kvt2` CAS + `OFFLINE_DRAIN_KVT2_ADVANCED` audit, all atomic) then Envelope 2 (`stage_finalize::run`).  NotFound/Mismatch from SentFresh context surface as `StructuralDrift` → `BootError::Internal`.
- **Commit 5 — Kvt1-source W12 wiring (Envelope 1b + Envelope 2)**: rewrite `process_via_w12_only` to invoke `kvt2_confirm::confirm_drain_doc(pool, dps, doc, doc.server_fiscal_no.as_deref().ok_or(StructuralDrift::ServerFiscalNoMissing)?, fn_sign, Kvt2ConfirmSource::Kvt1Reentry)` — `expected_server_fiscal_no` sourced from persisted doc row with structural-drift fail-loud on None (MED-PR70-R11-01 handoff); on `Acked` route through Envelope 1b (Kvt1Raw + `Kvt1 → Kvt2` + audit) then Envelope 2.  This is the **Kvt1 re-entry seam** for prior-tick-Held docs (HIGH-PR70-R2-01 fix).  NotFound/Mismatch from Kvt1Reentry context surface as `StructuralDrift` → `BootError::Internal`.
- **Commit 5b — Sent-replay W12 wiring** (HIGH-PR70-R3-01 + HIGH-PR70-R4-01 + MED-PR70-R5-02 + HIGH-PR70-R8-01 + MED-PR70-R11-01 fixes): rewrite `process_via_lastchk_replay` to invoke `kvt2_confirm::confirm_drain_doc(pool, dps, doc, doc.server_fiscal_no.as_deref().ok_or(StructuralDrift::ServerFiscalNoMissing)?, fn_sign, Kvt2ConfirmSource::SentReplay)` — `expected_server_fiscal_no` sourced from persisted doc row with structural-drift fail-loud on None (MED-PR70-R11-01 handoff); helper internally allocates the recovery `transport_trace` row in **Envelope 1c-pre** (via `transport_trace::allocate_and_insert_tx` per `boot_phase.rs:1521-1542`), then calls **`dps.by_server_fiscal_no(fn_sign, expected_server_fiscal_no)`** (HIGH-PR70-R8-01 canonical surface — NOT raw `last_chk` — to ensure empty-id → `Err(DpsError::NotFound)` mapping per `channel.rs:60-61` reaches the safe-redrive arm), and routes typed `Result<CheckAck, DpsError>` per the source-context matrix.  Match → Envelope 1a-replay (trace.complete OK + Kvt1Raw + Sent→Kvt1 + Kvt1→Kvt2 + audit) then Envelope 2 (`stage_finalize::run`).  NotFound → Envelope 1c-post (trace.complete TransientRetry + Sent→ER + audit) + `HoldFnDrain { projection: ErRedriveQueued }` (HIGH-C5-3 safe-redrive).  ServerFiscalIdMismatch → Envelope 1c-drift (trace.complete structural + audit) → BootError::Internal.  Transport/Server/Authorization/Decode/empty-data_sign → Envelope 1c-hold (trace.complete RetryableTransport|RetryableServer + audit) → `HoldFnDrain { projection: HeldAtSent }`.
...
- **Commit 8 — fixture acceptance**: 14 helper-typed + 15 drain-integration fixtures (matches Fixture matrix §"35 fixtures total" — 5 retained Hold variants after R5 moved LastChkIdMismatch → StructuralDrift; 1 Acked + 5 Hold + 2 StructuralDrift base + SentNotFoundDowngrade + NotFound × 3 contexts + Mismatch × 3 contexts).
...
7b. **Expected-id handoff per source (MED-PR70-R11-01 fix)**: `confirm_drain_doc` takes `expected_server_fiscal_no: &str` as a separate parameter from `doc: &DocumentRow`.  Caller sourcing:
  - SentFresh: `&outcome.server_fiscal_no` from `StageSendOutcome::Sent` (just-stamped fresh id; doc cohort snapshot is pre-stage_send and `doc.server_fiscal_no` is `None`).
  - SentReplay: `doc.server_fiscal_no.as_deref().ok_or(StructuralDrift::ServerFiscalNoMissing)?` (persisted Sent doc; None = structural drift).
  - Kvt1Reentry: same as SentReplay (Kvt1 doc must carry stamped id from prior Sent→Kvt1 advance).
  Helper trusts the parameter; does NOT inspect `doc.server_fiscal_no` for lookup.  Verified by `w12_sent_fresh_uses_outcome_server_fiscal_no_not_cohort_snapshot` fixture.
...
### Fixture matrix (35 fixtures total)
...
Drain integration — W0b latest-doc precondition proofs (HIGH-PR70-R2-01 + R3-01 + R4-01 CORE) — **17 fixtures** (including 5 SentReplay NotFound safe-redrive proofs added in R4 + 2 MED-PR70-R11-01 expected-id handoff proofs):
...
- `w12_sent_fresh_uses_outcome_server_fiscal_no_not_cohort_snapshot` (MED-PR70-R11-01 fix proof: helper receives expected_server_fiscal_no from `StageSendOutcome::Sent.server_fiscal_no` — not from pre-stage_send cohort `doc.server_fiscal_no`; asserts lookup proceeds with fresh id when cohort snapshot has `server_fiscal_no = None`)
- `w12_sent_replay_missing_server_fiscal_no_returns_structural_drift_pre_dps_call` (negative proof: persisted Sent doc with `server_fiscal_no = None` surfaces as `StructuralDrift(ServerFiscalNoMissing)` BEFORE any DPS call; no by_server_fiscal_no invocation; no trace allocation)
...
(Drain integration subtotal: 17 fixtures.)
