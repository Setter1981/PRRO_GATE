...
- **EDIT** `rust/prro/src/services/offline_sync/backlog_drain.rs`:
  ...
  - **Rewrite `process_via_lastchk_replay`** to be W12-aware (HIGH-PR70-R3-01 fix) while **preserving HIGH-C5-3 NotFound safe-redrive semantics** (HIGH-PR70-R4-01 reversal of prior R3 over-reach): on `DocState::Sent` cohort dispatch, delegate to `kvt2_confirm::confirm_drain_doc(..., Kvt2ConfirmSource::SentReplay)` which calls `DpsChannel::last_chk` **directly** and routes typed `Result<CheckAck, DpsError>` outcomes (MED-PR70-R7-01 fix: NOT through `last_chk_probe::ProbeOutcome` which collapses `DpsServer | DpsAuthorization | other` into `Unexpected{dps_error: String}` losing typed variants).  The legacy `last_chk_probe::probe` helper is retired for all three W12-aware drain entry points — it remains internal to other M3a/W9b code unaffected by W12.  Mapping:
    - `Ok(CheckAck)` with id == server_fiscal_no + non-empty `data_sign` → `Kvt2ConfirmOutcome::Acked { kvt1_raw_bytes, sent_replay_trace_attempt_no: Some(n) }` → Envelope 1a-replay chain (trace complete OK + Kvt1Raw + Sent→Kvt1 + Kvt1→Kvt2 + audit) then Envelope 2 (`stage_finalize::run`).  Audit `dispatch_via="kvt2_confirm_sent_replay"` for forensic distinction from Sent-fresh and Kvt1.  Summary `record_doc_advanced` with `via_lastchk_replay=true` (preserves W9b replay-flag semantic).
    - `Ok(CheckAck)` with id != server_fiscal_no → `Kvt2ConfirmOutcome::StructuralDrift { reason: LastChkIdMismatch, sent_replay_trace_attempt_no: Some(n) }` → Envelope 1c-drift (trace complete with structural failure evidence) → `BootError::Internal`.
    - `Ok(CheckAck)` with empty `data_sign` → `Kvt2ConfirmOutcome::Hold { reason: LastChkDataSignEmpty, sent_replay_trace_attempt_no: Some(n) }` → Envelope 1c-hold (trace complete no-state-change + audit) → `DocVerdict::HoldFnDrain { projection: HeldAtSent }`.
    - `Err(DpsError::NotFound)` (DPS has zero history; recognized via the not-found `DpsError` discriminant or its typed equivalent) → **`Kvt2ConfirmOutcome::SentNotFoundDowngrade { trace_attempt_no: n }`** (Sent-replay arm exclusively).  This is the safe Pattern B redrive case per W9b spec HIGH-C5-3 contract: the doc is missing on DPS side, repeated lastChk will never create the record — the recovery action is **resend through stage_send**, not poll forever.  Routing:
...
```json:metadata
{
  ...
  "operatorFindingsClosed": ["MED-PR70-01", "MED-PR70-02", "HIGH-PR70-R2-01", "MED-PR70-R2-02", "LOW-PR70-R2-03", "HIGH-PR70-R3-01", "MED-PR70-R3-02", "LOW-PR70-R3-03", "HIGH-PR70-R4-01", "MED-PR70-R5-01", "MED-PR70-R5-02", "MED-PR70-R6-01", "MED-PR70-R6-02", "LOW-PR70-R6-03", "MED-PR70-R7-01", "MED-PR70-R7-02", "LOW-PR70-R7-03"]
}
```
