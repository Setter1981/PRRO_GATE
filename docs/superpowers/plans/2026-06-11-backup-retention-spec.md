...
по соглашению имени файла; НИКОГДА не трогает файлы, не подходящие под наш
шаблон имени (чужие файлы в директории — не наши).

**ПОПРАВКА АРХИТЕКТОРА (2026-06-11, ревью PR-A).** Исходная формулировка
«удаляет сверх keep_last И старше max_age» была двусмысленной; имплементатор
честно реализовал букву (AND-delete), но AND-delete удерживает
`interval × max_age` полных копий БД (hourly × 14d ≈ 336 файлов на label) —
исчерпание диска на edge-железе. ЗАЛОЧЕНО: снапшот ЖИВ только пока он
ОДНОВРЕМЕННО в пределах `keep_last` свежайших И моложе `max_age_days`
(удаление по кэпу ИЛИ по экспирации). Безопасно: prune бежит только ПОСЛЕ
успешного снапшота — свежая копия переживает проход всегда.
