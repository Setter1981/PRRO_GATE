...
> **🏁 FINAL STATUS (2026-07-04) — this roadmap is COMPLETE; superseded by the pilot-path roadmap.** Phase 3 (oracle honesty) MERGED (#205/#206/#208/#209 — incl. the X1 prod runtime guard and generative crash-reach; C1 resolved as a verified no-op via CP-C1). Phase 4.1 WebCheck ground-truth **DONE end-to-end** (U0 #212 · U1 #213/#215/#214 · U2 #216/#217 · U3 #218): line-cited ground truth, derive-don't-adopt model (adoption-lint funnel, FORBIDDEN empty), sanitized hash-recomputed field corpus under a CP1 hard gate, and a directed differential replay harness whose **clean replay is GREEN — zero real divergences vs four years of field shapes** (intentional differences documented). Along the way: the fuzzer became an ENFORCED merge gate, the nightly caught+pinned find №3 (`apply_go_online`), and the musl CI-disk family was fixed. Remaining from this doc: **4.2 RETURN alphabet** (rolls into the successor) and the live-DPS campaign (parallel track). **The successor roadmap is `docs/superpowers/plans/2026-07-04-pilot-path-roadmap.md`** — frontier shifts from the fuzzer spine to the pilot path (A2.4 online activation → RETURN → live-DPS → pilot track).
