...
- **Ledger-only pin (M3b)** — `fiscal_documents` is a **ledger of issued receipts plus their non-issued *terminal* artifacts** — not a literal "issued-only" table. **Caveat (honest, corrected):** two refusal classes behave differently and the distinction matters for the oracle:
  - **Pre-acquire / invalid-ingress refusals** are rejected *before* an lnd/row is ever minted → they go to `audit_log` only, **never** to `fiscal_documents` (the literal "issued-only" wording holds here).
  - **Post-send DPS terminal rejects** act on a doc *already minted* into the ledger (PREPARED→SIGNED→SENDING): `AuthorizationKind::DocumentReject` CAS's the row `Sending → Rejected` (`error_routing.rs:295`, `TerminalReject`), so a **`Rejected` row legitimately rests in `fiscal_documents`** as a *non-issued* artifact (lnd consumed, **seed not advanced**, no offline code). The fuzzer model encodes exactly this (`model.rs:242`: minted row, `NoIssuanceRow`/no-seed-advance).

  The pin the #192 bug violated is therefore narrower than "no rejects in the ledger": it is **"no doc rests in a *non-terminal* state (`{PREPARED,SIGNED,ENCRYPTED}`) at a settled boundary."** An orphaned `SIGNED` row is illegal because `SIGNED` is non-terminal; a resting `Rejected` row is legal (it is terminal + non-issued). P1 (above, in the dry-run findings) is the boot-resume re-opening of this exact non-terminal-orphan hole.
