...
- **Insertion (corrected, audit #3):** NOT "the tail of `run_boot_reconciliation`" — it has multiple early-returns. Run the guard at the **App-level caller, after `run_boot_reconciliation` returns** (the post-reconciliation quiescent boundary), so every branch is covered by one check. **ШАГ-0 must pin the exact caller** (candidate: `App::reconcile_pending*` in `app.rs`) and confirm the guard runs once on the settled state regardless of which `BranchOutcome` returned.
...
- **Effect = CHECKPOINT (audit #3), not yet locked.** Default: audit `CRITICAL` + a **verifiable readiness effect**. ⚠ ШАГ-0 must **identify the real readiness surface** — the spec must not assume a `/health/ready` toggle that doesn't exist (the audit could not find one). **If a stuck non-terminal doc is unsafe to serve for that FN, a warning is NOT sufficient** — escalate to an **FN-level degraded/blocked readiness with a verifiable effect** (CP1). Decide WARN-vs-block *with the state-machine owner*, not by default.
