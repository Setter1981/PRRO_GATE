...
| `-2` | ERROR_CHECK | no — surfaces as `Server { code: -2, .. }` | `DpsError::Server { code: -2, .. }` | terminal-business | NO retry under M3 default. **Exception:** if doc_type ∈ {SHIFT_CLOSE, Z_REPORT} AND error_message indicates "open shift" → 1 `last_chk` reconciliation probe (WebCheck §1.2 row 5) | Sent → Rejected | RRO verification failed. WebCheck retries this only on close-shift edge (`SubmitPtr.cs:103-141`). Python: rejects terminal `dps_fiscal_server.py:512` |
...
| 12 | `-2` ERROR_CHECK | doc_type ∈ {SHIFT_CLOSE, Z_REPORT} + error_message includes "open shift" | one `last_chk` probe; route per outcome |
