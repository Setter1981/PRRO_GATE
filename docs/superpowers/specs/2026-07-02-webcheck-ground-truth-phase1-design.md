...
**Status:** **LOCKED (v2.1, 2026-07-03).** v1 → external wide audit (No-Go, 12 findings, all accepted; HIGH#1/#4 architect-verified vs C#) → v2 → re-check audit (9 CLOSED / 3 PARTIAL / 2 new MED, zero HIGH) → v2.1 closes the re-check gaps (D3/D5 concrete pins; adoption-lint wrapper-funnel + static-scan; exporter outside-tree output mandate; U0 citation-inventory enforcement). Implementer executes unit-by-unit per §3; checkpoints §7.
...
- **D3** — fork `OFFLINE_ISSUED_STATES` into a model literal + `debug_assert` equality. **Grounding: the prod const `fiscal_documents::OFFLINE_ISSUED_STATES` (`rust/prro/src/db/repositories/fiscal_documents.rs`) + the `DocState` enum boundary** — the fork asserts against that exact const (anti-shared-const; dry-run D3).
- **D4** — `BadHashPrev`: apply the no-resend bound (minimum) or predict the single-shot-stub terminal. **Grounding: our write-path MAC-recovery semantics — the W10.4 single-re-entry dispatch budget + the DDL `mac_recovery_attempts CHECK IN (0,1)` bound (`stage_send.rs`).**
- **D5** — promote the deterministic exotic-drain scripts (Superseded→all ERROR_RETRYABLE; NotFound-hold→SENT) to predicted `Mutated`; **MAC-recovery labeled genuinely deferred**. **Grounding: the M3b W9b strict-sequential drain contract (`docs/superpowers/specs/2026-05-17-m3b-shift-state-expansion.md` §16.7 + `backlog_drain.rs` per-doc loop) and the drain classifier semantics (`kvt2_confirm::classify_check_result`)** — the exact terminals these scripts pin are already asserted by the Cluster-B drain tests.
- RED-first + paired negative teeth per closure (false-positive = merge-blocker on the enforced gate).
- **A1 is mechanical (audit MED#9 + re-check F9): an adoption-lint with an enforced funnel.** All DB access in `model.rs` MUST go through tagged wrapper fns (`adopt_fault_deferred(…)`, `adopt_precondition(…)`, `read_seed_fixture(…)`) — raw `sqlx`/`.fetch_*` calls outside the wrappers are forbidden. Exhaustiveness is enforced by a **static-scan test**: it reads `model.rs` as text and FAILS on any `sqlx::`/`query`/`fetch_` token outside the wrapper definitions (the same source-scan pattern as the existing compile-fail/guard tests). The registry classifies each wrapper call-site `{seed-fixture | fault-deferred | precondition-only}`; FORBIDDEN = any unclassified/raw read = scan failure. A1 passes iff the scan is green and every closure's teeth pair is green.

### U2 — Sanitized, hash-recomputed corpus — ⚠ DATA-SENSITIVE (CP1 hard gate + mechanical gate)

- **Raw dumps:** operator-provided; **outside the repo tree (never transiting it), never committed, destroyed after export** (audit C3/C6).
  - **Operational note (2026-07-03): dumps DELIVERED.** Location: **`~/webcheck_dumps/`** (WSL-local ext4, OUTSIDE the repo; dir `700`, files `600`; local `MANIFEST.txt` with sizes/copy-date — the manifest stays local, it is not repo material). Source: operator's `/mnt/d/db` (Windows drive), copied 2026-07-03; **31 files ≈ 734 MB ≈ 15 fiscal numbers** (per-FN `<FN>.db` + `<FN>_TS.db` companion pairs — the `_TS` companion's role is a U0 question). Copy verified file-set-identical to the source. Encryption-at-rest on the dev box: NOT applied (single-user WSL, permission-restricted) — accepted deviation, revisit if the box is shared. U2's export stage reads from this path; nothing under `~/webcheck_dumps/` may ever be copied into the repo tree, even transiently.
...
- **Exporter output path (re-check F7 / new-MED#2):** the exporter's DEFAULT output (`var/webcheck_samples/` — INSIDE the repo tree, gitignored-but-transiting) **violates the never-transit rule**. U2 mandates: (a) every export run targets an outside-tree dir (`~/webcheck_dumps/exports/`); (b) as a U2 task, change `export_webcheck_samples.py` to REQUIRE an explicit `--output` (no repo-local default); (c) the corpus scanner additionally fails if `var/webcheck_samples/` exists in the working tree.
...
- **A0:** `WEBCHECK_GROUND_TRUTH.md` exists; DDL + offline-lifecycle + per-lane MAC + lnd-translation + `fns` tables each carry C# citations; anchors machine-regenerated (no approximate counts). **U0-gate enforcement (re-check new-MED#1):** each later unit's Delivery includes a **citation inventory** — every WebCheck-behavior claim it relies on, cross-checked line-by-line against `WEBCHECK_GROUND_TRUTH.md` by the reviewing architect; a claim absent from U0's doc blocks the unit (extend U0 first).
