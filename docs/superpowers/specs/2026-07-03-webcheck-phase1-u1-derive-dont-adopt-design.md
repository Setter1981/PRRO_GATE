...
1. **D5 predicted terminal — RULED: pin to the REAL `classify_check_result` arms.** The parent §3/dry-run shorthand ("Superseded→all ERROR_RETRYABLE; NotFound-hold→SENT") is **superseded** — it inherited the dry-run's stale wording; the verified arms are `SupersededHold` (doc held in SENT) / `SentNotFoundDowngrade` / `StructuralDrift`. Cohort semantics honor the strict-sequential STOP (`backlog_drain.rs:928-946`) and the RMR halt (`:725`). The precise model-level `Mutated` shape per script is derived during RED from these arms + the drain loop (and reviewed migration-grade) — the RULING fixes the source of truth (real behavior), not a guessed table. **Empirical outcome (execution, architect-verified vs `backlog_drain.rs:1041-1059`):** in the DRAIN context `SupersededHeld` deliberately REVERSES sibling-continue (M2-N1 contract + ruling B: a superseded predecessor would wedge if merely held) → **halt + `escalate_drain_to_manual` (shift→RMR), head doc → `ErrorRetryable`**; `[Ack, NotFound]` → head held `SENT`, shift unchanged. The illustrative "held in SENT, siblings continue" wording above described the kvt2-confirm arm in isolation, not the drain-cohort semantics — superseded by this outcome.
