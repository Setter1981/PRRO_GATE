...
- **Audit-cardinality guard for sustained outages** (W8a Round 3 review L2): at the W8a default 60s cadence, a multi-hour DPS outage produces ~60 `RETURN_ONLINE_PROBE_FAILED` rows per hour per FN.  Auditable by design, but at scale this floods the audit log and dilutes signal.  W8b adds rate-limiting / dedup: collapse consecutive same-class `_FAILED` rows (same `dps_error_class` *and* same `reason`) into a single durable row carrying `first_failure_at` + `last_failure_at` + `consecutive_count`, refreshed every M ticks (configurable, default 10 — emits one durable record per ~10 min at the W8a default cadence).  First failure of a new class always emits a fresh row.  Mode flip back to `Online` resets the dedup state.  Implementation lives at the loop level, not the primitive — `run_tick_for_fn` continues to emit per-tick `_FAILED` and the loop wrapper applies the dedup.
- **Module doc cross-link** (W8a Round 3 review L3): add a one-paragraph module-doc note to `services/offline_sync/return_online_probe.rs` explaining the `statusRro` vs `ping` design choice (rationale: `statusRro` gives a read-only surface AND records `open_shift` / `last_signer` for forensics in the success/failure audit; `ping` was the lighter alternative but does not carry the snapshot fields, and the snapshot is operator-pinned hard line 2).  Cross-link to design freeze §10 + memory `m3b-w8-review-criteria` axis 6.  One-line in module doc; not load-bearing on runtime behaviour but resists future drift toward the lighter call.
