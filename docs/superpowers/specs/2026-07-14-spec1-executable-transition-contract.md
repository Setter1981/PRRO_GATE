...
| Sent | BootRecover{probe=NotFound} | boot SENT probe: DPS no record (ProbeOutcome::NotFound); tick-1 of 2; CAS from=SENT | ErrorRetryable | SENT→ERROR_RETRYABLE; complete trace RetryableServer/TransientRetry; audit BOOT_SENT_LAST_CHK_NOTFOUND; tick-2 re-drives Pattern B | no | whitelist fiscal_documents.rs:190; boot_phase.rs:964 |
| Sent | Drain{confirm,lastChk NotFound} (offline drain) | offline drain lastChk: DPS no record; CAS from=SENT | ErrorRetryable | SENT→ERROR_RETRYABLE; complete trace TransientRetry; audit; re-drive next drain tick | no | whitelist fiscal_documents.rs:190; offline_sync/kvt2_confirm.rs:1696 |
