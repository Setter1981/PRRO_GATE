...
5. **Hard close-code reserve = 1 offline code per active offline shift.**  The offline Z_REPORT close path described in (2) needs an offline code at the moment of emission.  If ordinary offline `SELL` / `RETURN` / `SERVICE_*` docs are allowed to consume the entire pool while a shift is open, the operator reaches close-of-day with zero free codes — and the Pattern C local Z_REPORT close-of-day exit becomes inaccessible, re-asserting the 24h trap.  Therefore W10 enforces a hard reserve of exactly one offline code per active offline shift: ordinary fiscal docs refuse-with-audit (`OFFLINE_CODE_RESERVED_FOR_CLOSE`) when only the reserved code remains; the offline Z_REPORT MAY consume it.  This is a **legal escape hatch**, not an operational watermark — the latter (`min_offline_codes`, commonly set ~10 by operators) is the *upstream* refill trigger; the hard reserve is the *last-line* legal guarantee.  Pool=0 at close-of-day time is pilot-critical / legal-critical: the offline Z_REPORT itself is refused with `OFFLINE_Z_REPORT_LOCAL_CLOSE_REFUSED` + `reason: "code_pool_exhausted"`, and the operational refill watermark has failed upstream.
