-- Migration 003: Add OFFLINE_LOCAL_ACK to fiscal_documents.state CHECK constraint.
--
-- Background: offline-processed documents were incorrectly stored with state='ACK',
-- implying final DPS acknowledgement. OFFLINE_LOCAL_ACK is the correct terminal state
-- for locally accepted offline documents pending DPS sync.
--
...
-- migrates existing misclassified offline ACK rows, and rebuilds all indexes.
--
-- Data migration rule:
--   state='ACK' AND fs_mode='OFFLINE' AND submission_status='OFFLINE_LOCAL'
...
-- NOTE: The DDL below must remain in sync with 001_hot_store_init.sql.
-- Any future migration that alters fiscal_documents must also update this block
-- if 003 can ever be re-applied (e.g. on a pre-003 database or test fixture).
