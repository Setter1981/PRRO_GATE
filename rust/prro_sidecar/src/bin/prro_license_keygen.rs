...
#[derive(Parser)]
#[command(name = "prro_license_keygen", about = "Generate DSTU master keypair for license signing")]
struct Cli {
...
fn main() {
    ...
    let d = loop {
        let mut buf = [0u8; 32];
        rand_core::OsRng.try_fill_bytes(&mut buf)
    ...
    for (wi, &word) in d.bytes.iter().enumerate() {
        for bi in 0..4usize {
            let idx = wi * 4 + bi;
            if idx < 32 { priv_bytes[idx] = (word >> (bi * 8)) as u8; }
        }
    }

    // Write
    std::fs::write(&cli.out_priv, &priv_bytes)
        .unwrap_or_else(|e| { eprintln!("error writing {:?}: {e}", cli.out_priv); std::process::exit(1); });
    std::fs::write(&cli.out_pub, &pub_compressed)
        .unwrap_or_else(|e| { eprintln!("error writing {:?}: {e}", cli.out_pub); std::process::exit(1); });

    println!("Private key → {:?} (32 bytes)", cli.out_priv);
    println!("Public key  → {:?} (33 bytes, compressed DSTU PB-257)", cli.out_pub);
}
