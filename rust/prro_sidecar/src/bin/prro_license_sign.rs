...
#[command(
    ...
    about = "Sign license payloads with DSTU master key"
)]
struct Cli {
...
fn parse_tier(s: &str) -> Result<LicenseTier, String> {
    match s {
        ...
        other => Err(format!(
            "unknown tier '{other}'; choose demo|basic|pro|enterprise"
        )),
    }
}
...
fn load_private_key(path: &PathBuf) -> Vec<u8> {
    let bytes = std::fs::read(path).unwrap_or_else(|e| {
        eprintln!("error reading master key {:?}: {e}", path);
        std::process::exit(1);
    });
    if bytes.len() != 32 {
        eprintln!(
            "error: master key must be exactly 32 bytes, got {}",
            bytes.len()
        );
        std::process::exit(1);
    }
...
fn build_single_payload(cli: &Cli) -> LicensePayload {
    let tin = cli.tin.clone().unwrap_or_else(|| {
        eprintln!("--tin required in single mode");
        std::process::exit(1);
    });
    let fn_numbers = if cli.fn_numbers.is_empty() {
        eprintln!("at least one --fn required");
        std::process::exit(1);
    } else {
        cli.fn_numbers.clone()
    };
    let expires_at = cli.expires_at.clone().unwrap_or_else(|| {
        eprintln!("--expires-at required");
        std::process::exit(1);
    });
    let tier = cli.tier.unwrap_or_else(|| {
        eprintln!("--tier required");
        std::process::exit(1);
    });

    // Refuse past-dated expiry
    let exp = time::OffsetDateTime::parse(
        &format!("{expires_at}T00:00:00Z"),
        &time::format_description::well_known::Rfc3339,
    )
    .unwrap_or_else(|e| {
        eprintln!("invalid --expires-at '{expires_at}': {e}");
        std::process::exit(1);
    });
...
fn process_csv(csv_path: &PathBuf, out_dir: &PathBuf, d_bytes: &[u8]) {
    let content = std::fs::read_to_string(csv_path).unwrap_or_else(|e| {
        eprintln!("error reading CSV {:?}: {e}", csv_path);
        std::process::exit(1);
    });
    std::fs::create_dir_all(out_dir).unwrap_or_else(|e| {
        eprintln!("error creating out-dir {:?}: {e}", out_dir);
        std::process::exit(1);
    });
    ...
    for (i, line) in lines.enumerate() {
        ...
        let fn_numbers: Vec<String> = fields[1]
            .trim()
            .trim_matches('"')
            .split(';')
            .map(str::trim)
        ...
        let tier = parse_tier(fields[3].trim()).unwrap_or_else(|e| {
            eprintln!("row {}: {e}", i + 2);
            std::process::exit(1);
        });
        let org_name = fields
...
fn main() {
    ...
    if let Some(csv_path) = &cli.from_csv {
        let out_dir = cli.out_dir.clone().unwrap_or_else(|| {
            eprintln!("--out-dir required with --from-csv");
            std::process::exit(1);
        });
        ...
        let out = cli.out.clone().unwrap_or_else(|| {
            eprintln!("--out required in single mode");
            std::process::exit(1);
        });
