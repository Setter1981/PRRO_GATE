...
#[command(
    ...
    about = "Verify a license file against the embedded master pubkey"
)]
struct Cli {
...
fn main() {
    ...
    let file_content = std::fs::read_to_string(&cli.license).unwrap_or_else(|e| {
        eprintln!("error reading {:?}: {e}", cli.license);
        std::process::exit(2);
    });

    let doc: serde_json::Value = serde_json::from_str(&file_content).unwrap_or_else(|e| {
        eprintln!("JSON parse error: {e}");
        std::process::exit(2);
    });

    let payload_b64 = doc["payload_b64"].as_str().unwrap_or_else(|| {
        eprintln!("missing payload_b64");
        std::process::exit(2);
    });
    let signature_b64 = doc["signature_b64"].as_str().unwrap_or_else(|| {
        eprintln!("missing signature_b64");
        std::process::exit(2);
    });

    // Parse the payload JSON for metadata to include in output
    let payload_bytes = B64.decode(payload_b64).unwrap_or_else(|e| {
        eprintln!("base64 decode error: {e}");
        std::process::exit(2);
    });
    let payload: LicensePayload = serde_json::from_slice(&payload_bytes).unwrap_or_else(|e| {
        eprintln!("payload JSON error: {e}");
        std::process::exit(2);
    });
    ...
    let tier_str = serde_json::to_value(payload.tier)
        .unwrap()
        .as_str()
        .unwrap_or("unknown")
        .to_string();
    ...
    let (output, exit_code) = match &state {
        ...
        LicenseState::FnNotLicensed { fn_ } => (json!({"state":"FnNotLicensed","fn":fn_}), 1),
        LicenseState::SignatureInvalid => (json!({"state":"SignatureInvalid"}), 1),
    };
