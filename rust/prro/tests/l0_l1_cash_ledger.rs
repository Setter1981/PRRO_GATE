...
fn pin_l0_mixed_receipt() {
    ...
    assert_eq!(derive_cash_on_hand(0, 4000, 0), 4000);
}
...
async fn pin_l0_aggregate_shift_cash_two_sells() {
    ...
    let (sell, ret) = aggregate_shift_cash(&pool, FN, shift).await.unwrap();
...
async fn pin_l0_aggregate_mixed_receipt() {
    ...
    let (sell, _) = aggregate_shift_cash(&pool, FN, shift).await.unwrap();
