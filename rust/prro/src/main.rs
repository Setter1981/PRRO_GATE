...
enum AdminCmd {
    ...
    AddTaxGroup {
        ...
        #[arg(long = "fn")] fiscal_number: String,
        #[arg(long)] tx_num: i64,
        #[arg(long)] letter: String,
        #[arg(long, default_value_t = 0.0)] dtpr: f64,
        #[arg(long, default_value_t = 0.0)] txpr: f64,
        #[arg(long, default_value_t = 0)]   txal: i64,
    },
    ...
    UpdateTaxRate {
        ...
        #[arg(long = "fn")] fiscal_number: String,
        #[arg(long)] tx_num: i64,
        #[arg(long)] dtpr: Option<f64>,
        #[arg(long)] txpr: Option<f64>,
        #[arg(long)] txal: Option<i64>,
    },
    ...
    AddPayment {
        #[arg(long)] config: PathBuf,
        #[arg(long = "fn")] fiscal_number: String,
        #[arg(long)] pay_index: i64,
        #[arg(long)] name: String,
        /// 1 = cash, 0 = cashless.  Default cashless.
        #[arg(long, default_value_t = false)] cash: bool,
    },
    ...
    AddDriverMapping {
        #[arg(long)] config: PathBuf,
        #[arg(long)] driver_id: String,
        #[arg(long)] driver_number: i64,
        #[arg(long)] canonical: i64,
        #[arg(long)] letter: Option<String>,
    },
...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.cmd {
        ...
        Cmd::Admin { cmd } => match cmd {
            ...
            AdminCmd::AddTaxGroup { config, fiscal_number, tx_num, letter, dtpr, txpr, txal } => {
                w4z0_dispatch(prro::admin_w4_z0::run_add_tax_group(
                    &config, fiscal_number, tx_num, letter, dtpr, txpr, txal,
                ).await, "add-tax-group")
            }
            AdminCmd::UpdateTaxRate { config, fiscal_number, tx_num, dtpr, txpr, txal } => {
                w4z0_dispatch(prro::admin_w4_z0::run_update_tax_rate(
                    &config, fiscal_number, tx_num, dtpr, txpr, txal,
                ).await, "update-tax-rate")
            }
            AdminCmd::RemoveTaxGroup { config, fiscal_number, tx_num } => {
                w4z0_dispatch(prro::admin_w4_z0::run_remove_tax_group(
                    &config, fiscal_number, tx_num,
                ).await, "remove-tax-group")
            }
            AdminCmd::ListTaxGroups { config, fiscal_number } => {
                match prro::admin_w4_z0::run_list_tax_groups(&config, fiscal_number).await {
                    ...
                    Err(err) => { eprintln!("prro admin list-tax-groups: {err}"); std::process::exit(err.exit_code()); }
                }
            }

            AdminCmd::AddPayment { config, fiscal_number, pay_index, name, cash } => {
                w4z0_dispatch(prro::admin_w4_z0::run_add_payment_method(
                    &config, fiscal_number, pay_index, name, cash,
                ).await, "add-payment")
            }
            AdminCmd::UpdatePayment { config, fiscal_number, pay_index, name, cash } => {
                w4z0_dispatch(prro::admin_w4_z0::run_update_payment_method(
                    &config, fiscal_number, pay_index, name, cash,
                ).await, "update-payment")
            }
            AdminCmd::RemovePayment { config, fiscal_number, pay_index } => {
                w4z0_dispatch(prro::admin_w4_z0::run_remove_payment_method(
                    &config, fiscal_number, pay_index,
                ).await, "remove-payment")
            }
            AdminCmd::ListPayments { config, fiscal_number } => {
                match prro::admin_w4_z0::run_list_payment_methods(&config, fiscal_number).await {
                    ...
                    Err(err) => { eprintln!("prro admin list-payments: {err}"); std::process::exit(err.exit_code()); }
                }
            }

            AdminCmd::SetFlag { config, fiscal_number, name, value } => {
                w4z0_dispatch(prro::admin_w4_z0::run_set_flag(
                    &config, fiscal_number, name, value,
                ).await, "set-flag")
            }
            AdminCmd::SetNationalReceipt { config, fiscal_number, enabled } => {
                w4z0_dispatch(prro::admin_w4_z0::run_set_national_receipt(
                    &config, fiscal_number, enabled,
                ).await, "set-national-receipt")
            }
            AdminCmd::ListFlags { config, fiscal_number } => {
                match prro::admin_w4_z0::run_list_flags(&config, fiscal_number).await {
                    ...
                    Err(err) => { eprintln!("prro admin list-flags: {err}"); std::process::exit(err.exit_code()); }
                }
            }

            AdminCmd::AddDriverMapping { config, driver_id, driver_number, canonical, letter } => {
                w4z0_dispatch(prro::admin_w4_z0::run_add_driver_mapping(
                    &config, driver_id, driver_number, canonical, letter,
                ).await, "add-driver-mapping")
            }
            AdminCmd::UpdateDriverMapping { config, driver_id, driver_number, canonical } => {
                w4z0_dispatch(prro::admin_w4_z0::run_update_driver_mapping(
                    &config, driver_id, driver_number, canonical,
                ).await, "update-driver-mapping")
            }
            AdminCmd::RemoveDriverMapping { config, driver_id, driver_number } => {
                w4z0_dispatch(prro::admin_w4_z0::run_remove_driver_mapping(
                    &config, driver_id, driver_number,
                ).await, "remove-driver-mapping")
            }
            AdminCmd::ListDriverMappings { config, driver_id } => {
                match prro::admin_w4_z0::run_list_driver_mappings(&config, driver_id).await {
                    ...
                    Err(err) => { eprintln!("prro admin list-driver-mappings: {err}"); std::process::exit(err.exit_code()); }
                }
            }

            AdminCmd::SetOutgressProfile { config, fiscal_number, profile } => {
                w4z0_dispatch(prro::admin_w4_z0::run_set_outgress_profile(
                    &config, fiscal_number, profile,
                ).await, "set-outgress-profile")
            }
            AdminCmd::ShowOutgressProfile { config, fiscal_number } => {
                match prro::admin_w4_z0::run_show_outgress_profile(&config, fiscal_number).await {
                    Ok(profile) => { println!("profile={}", profile.as_db_str()); Ok(()) }
                    Err(err) => { eprintln!("prro admin show-outgress-profile: {err}"); std::process::exit(err.exit_code()); }
                }
            }

            AdminCmd::BootstrapDefaults { config, fiscal_number } => {
                w4z0_dispatch(prro::admin_w4_z0::run_bootstrap_defaults(
                    &config, fiscal_number,
