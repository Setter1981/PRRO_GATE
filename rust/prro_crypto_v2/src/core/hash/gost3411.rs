...
use crate::core::hash::gost28147::Gost;
...
/// XOR two 32-byte blocks (u64-wide).
#[inline]
fn xor_blocks(out: &mut [u8; 32], a: &[u8; 32], b: &[u8; 32]) {
    for i in 0..4 {
        ...
        out[off..off + 8].copy_from_slice(&(wa ^ wb).to_le_bytes());
    }
}
...
///
/// Shift data left by 2 bytes, put LFSR feedback at the end.
/// Reads all source words before writing to avoid aliasing.
#[inline]
fn transform_3(data: &mut [u8; 32]) {
    ...
    // Read 30 source bytes (data[2..32]) as wide words before overwriting
    let a = u64::from_le_bytes(data[2..10].try_into().unwrap());
    let b = u64::from_le_bytes(data[10..18].try_into().unwrap());
    let c = u64::from_le_bytes(data[18..26].try_into().unwrap());
    let d = u32::from_le_bytes(data[26..30].try_into().unwrap());
    let e = u16::from_le_bytes(data[30..32].try_into().unwrap());
    // Write back shifted
    data[0..8].copy_from_slice(&a.to_le_bytes());
    data[8..16].copy_from_slice(&b.to_le_bytes());
    data[16..24].copy_from_slice(&c.to_le_bytes());
    data[24..28].copy_from_slice(&d.to_le_bytes());
    data[28..30].copy_from_slice(&e.to_le_bytes());
    data[30..32].copy_from_slice(&t.to_le_bytes());
}

/// add_blocks: add two 32-byte little-endian big-integers (u64-wide with carry).
...
fn add_blocks(left: &mut [u8; 32], right: &[u8; 32]) {
    ...
    for i in 0..4 {
        let off = i * 8;
        let a = u64::from_le_bytes(left[off..off + 8].try_into().unwrap());
        let b = u64::from_le_bytes(right[off..off + 8].try_into().unwrap());
        let (sum1, c1) = a.overflowing_add(b);
        let (sum2, c2) = sum1.overflowing_add(carry);
        left[off..off + 8].copy_from_slice(&sum2.to_le_bytes());
