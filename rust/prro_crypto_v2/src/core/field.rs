...
impl FieldEl {
    ...
    /// True if this element equals 1 (the multiplicative identity).
    pub fn is_one(&self) -> bool {
        ...
        self.bytes[1..].iter().all(|&w| w == 0)
    }

    /// True if this element is in GF(2), i.e. its value is 0 or 1.
    /// Required for Koblitz-curve detection: b must be in GF(2) for τ-NAF.
    pub fn is_gf2_element(&self) -> bool {
        if self.bytes.is_empty() {
            return true;
        }
        let low = self.bytes[0];
        if low != 0 && low != 1 {
            return false;
        }
        self.bytes[1..].iter().all(|&w| w == 0)
