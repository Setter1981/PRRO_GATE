...
fn sell_with_items_payments_slip_lands_canonical_envelope_on_bridge() {
    ...
    run(
        ...
        &bridge,
        &mut correlation,
        Command::Prep("Bar1".to_string()),
    );

    // Three fiscal lines — bodies are opaque to Rust, the Python
    // adapter does the rich parse.
    for body in ["goods_line_1", "goods_line_2", "goods_line_3"] {
        run(
            &mut session,
            &bridge,
            &mut correlation,
            Command::Fisc(body.to_string()),
        );
    }
    ...
    run(
        ...
        &bridge,
        &mut correlation,
        Command::Acld("03STAMP_001".to_string()),
    );
    ...
    run(
        ...
        &bridge,
        &mut correlation,
        Command::Psdt("102merchantTerm".to_string()),
    );
...
fn idempotency_key_is_content_stable() {
    ...
    run(
        ...
        &bridge,
        &mut correlation,
        Command::Fisc("100item".to_string()),
    );
    run(
        &mut session,
        &bridge,
    ...
    run(
        ...
        &bridge,
        &mut correlation,
        Command::Prep("Y".to_string()),
    );
    run(
        &mut session,
        &bridge,
        &mut correlation,
        Command::Fisc("200item".to_string()),
    );
    run(
        &mut session,
        &bridge,
    ...
    assert_ne!(
        key_a1, key_b,
        "different receipt content must produce different idempotency keys"
    );
...
fn comp_retry_after_bridge_failure_submits_same_canonical_envelope() {
    ...
    assert!(
        session.receipt_open(),
        "receipt must stay open after failure"
    );
...
fn session_psdt_sequence_stays_in_sync_with_receipt_psdt_sequence() {
    ...
    match &session.state {
        maria304_driver::session::SessionState::ReceiptOpen(r) => {
            assert_eq!(
                r.psdt_sequence, 3,
                "OpenReceipt counter must mirror Session"
            );
        }
        other => panic!("{other:?}"),
    }
...
fn cancelled_then_reopened_receipt_starts_with_fresh_raw_frames() {
    ...
    run(
        ...
        &bridge,
        &mut correlation,
        Command::Fisc("leftover".to_string()),
    );
    run(&mut session, &bridge, &mut correlation, Command::Cnac);
...
fn return_line_opcode_marks_receipt_as_return() {
    ...
    run(
        ...
        &bridge,
        &mut correlation,
        Command::Bfis("return-line".to_string()),
    );
    run(
        &mut session,
        &bridge,
...
fn comp_accepted_closes_receipt() {
    ...
    assert!(
        responses
            .iter()
            .any(|r| matches!(r, Response::Error(ErrorCode::SoftNoDoc))),
