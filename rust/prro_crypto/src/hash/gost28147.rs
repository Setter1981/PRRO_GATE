...
impl Gost {
    ...
    fn with_unpacked_sbox(sbox: &[u8; 128]) -> Self {
    ...
    pub fn encrypt64(&self, clear: &[u8; 8], out: &mut [u8; 8]) {
        let mut n0 = (clear[0] as u32)
            | ((clear[1] as u32) << 8)
            | ((clear[2] as u32) << 16)
            | ((clear[3] as u32) << 24);
        let mut n1 = (clear[4] as u32)
            | ((clear[5] as u32) << 8)
            | ((clear[6] as u32) << 16)
            | ((clear[7] as u32) << 24);
        ...
        // Output: swap halves back (n1 low, n0 high) — note jkurwa writes
        // `out[0..4] = n[1]`, `out[4..8] = n[0]` which matches Feistel final swap.
        out[0] = (n1 & 0xFF) as u8;
        out[1] = ((n1 >> 8) & 0xFF) as u8;
        out[2] = ((n1 >> 16) & 0xFF) as u8;
        out[3] = ((n1 >> 24) & 0xFF) as u8;
        out[4] = (n0 & 0xFF) as u8;
        out[5] = ((n0 >> 8) & 0xFF) as u8;
        out[6] = ((n0 >> 16) & 0xFF) as u8;
        out[7] = ((n0 >> 24) & 0xFF) as u8;
    }
}
