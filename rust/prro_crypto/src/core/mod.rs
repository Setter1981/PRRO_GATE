...
// Lint-debt pass CRY-4 (2026-06-10), DELIBERATE allow: explicit index
// loops in this module tree are constant-time table walks and word-level
// field/hash arithmetic (gf2m/fe/point/comb/mladder/kupyna/…). Rewriting
// them into iterator chains to satisfy these style lints would obscure
// exactly the structure a constant-time audit (and the dudect tests)
// reasons about, for zero semantic gain — per the house rule "explicit
// code in hot paths". Everything else in the crate is lint-clean and
// gated by `-D warnings` in CI; do NOT widen this allow beyond `core`.
