...
fn ymd_hms_rejects_impossible_day_of_month() {
    ...
    assert!(
        ymd_hms_to_unix(2016, 12, 31, 23, 59, 60).is_some(),
        "leap second 60"
    );
    assert!(
        ymd_hms_to_unix(2016, 12, 31, 23, 59, 61).is_none(),
        "second 61"
    );
}
...
fn parse_asn1_time_rejects_trailing_junk_and_impossible_calendar() {
    ...
    assert!(
        parse_asn1_time(b"491231235959junkZ", 0x17).is_err(),
        "UTCTime trailing junk"
    );
    assert!(
        parse_asn1_time(b"20491231235959junkZ", 0x18).is_err(),
        "GeneralizedTime trailing junk"
    );
    // Fractional seconds (forbidden in RFC 5280 cert validity) — rejected.
    assert!(
        parse_asn1_time(b"20491231235959.5Z", 0x18).is_err(),
        "GeneralizedTime fractional"
    );
    ...
    assert!(
        parse_asn1_time(b"490231235959Z", 0x17).is_err(),
        "Feb 31 (2049-02-31)"
    );
    assert!(parse_asn1_time(b"491231255959Z", 0x17).is_err(), "hour 25");
    assert!(
        parse_asn1_time(b"491231236099Z", 0x17).is_err(),
        "minute 60"
    );
    // Still rejects missing-Z / non-digit.
    assert!(
        parse_asn1_time(b"491231235959", 0x17).is_err(),
        "no trailing Z"
    );
    assert!(
        parse_asn1_time(b"4912312359XXZ", 0x17).is_err(),
        "non-digit time field"
    );
}
